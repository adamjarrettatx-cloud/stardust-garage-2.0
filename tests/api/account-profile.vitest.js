...
describe('own contact-details mutation', () => {
  ...
  it.each(['+12025550199', ''])('clears verification when the phone changes to %s', async (phone) => {
    expect((await PATCH(request({ fullName: 'Test Person', phone }))).status).toBe(200);
  ...
  it.each([{ fullName: '', phone: '' }, { fullName: 'x'.repeat(121), phone: '' }, { fullName: 'Test', phone: '' }, { fullName: 'Test Person', phone: 'not a phone' }, { fullName: 'Test Person', phone: '123' }, { fullName: 'Test Person', phone: 123 }])('rejects invalid contact values', async (body) => {
  ...
  it('accepts authenticated mobile bearer calls without ambient origin', async () => {
    expect((await PATCH(request({ fullName: 'Test Person', phone: '' }, null, 'Bearer verified-by-auth-helper')))).toHaveProperty('status', 200);
  });
  it('does not overwrite data after a failed read or leak database details', async () => {
    failure = true;
    const response = await PATCH(request({ fullName: 'Test Person', phone: '' }));
