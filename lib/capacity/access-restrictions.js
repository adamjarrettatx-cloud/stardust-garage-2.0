...
export async function resolveAccessSubject(admin, subject) {
  ...
  // A corrected name must not hide a ban or possible match under a previous
  // name. Keep aliases as advisory matches, never automatically confirmed.
  const { data: corrections, error: correctionError } = await admin.from('legal_name_corrections')
    .select('old_names').overlaps('identity_keys', identity.identity_keys).limit(1000);
  if (correctionError || !corrections || corrections.length >= 1000) throw new Error('Name history unavailable.');
  for (const correction of corrections) {
    for (const full_name of correction.old_names || []) identity.match_keys.push(...contactKeys({ full_name }));
  }
