...
import { DOOR_RESULTS, evaluateDoorScan, isPassLive } from '@/lib/trial-pass';
...
export async function hydratePeople(admin, seeds, arrivals, context, since) {
  ...
  const shape = async group => {
    ...
    if (primary.kind === 'trial_pass') {
      ...
      // A missing selfie does not block a roster arrival either; staff see the
      // person in front of them. Expiry still applies. The photo gate remains
      // on QR activation, which this path never performs.
      const decision = evaluateDoorScan({ pass: primary, event: null });
      if (!decision.allowed && decision.result !== DOOR_RESULTS.denied_no_photo) reason = decision.reason;
