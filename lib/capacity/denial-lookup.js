...
// fetchDoorSessionStart(admin, doorSessionId) -> number | null
//
// When the door session opened, in epoch ms. This is what lets the prior-denial
// banner say "turned away twice TONIGHT" instead of the much weaker "twice
// before" -- the count was always right, but "before" reads like ancient
// history when it actually happened forty minutes ago at this same door.
//
// The scan endpoints receive a door_session_id in the request body and nothing
// else about the session, so the start time is one small lookup. Returns null
// on any failure or when no session is open, in which case the banner falls
// back to all-time wording rather than misattributing old denials to tonight.
export async function fetchDoorSessionStart(admin, doorSessionId) {
  if (!admin || typeof doorSessionId !== 'string' || !doorSessionId) return null;
  const { data, error } = await admin
    .from('door_sessions')
    .select('opened_at')
    .eq('id', doorSessionId)
    .maybeSingle();
  if (error || !data?.opened_at) return null;
  const ms = new Date(data.opened_at).getTime();
