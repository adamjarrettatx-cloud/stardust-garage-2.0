...
export function capacityHourLabel(at, intervalMinutes = 60) {
...
export function buildCapacityReport({ date, sessions = [], rows = [], seeds = [], now = Date.now(), intervalMinutes = 60 }) {
  if (![5, 60].includes(intervalMinutes)) throw new Error('Invalid capacity interval');
  ...
  for (let at = start; at < end; at += step) {
    const until = Math.min(at + step, end);
    ...
    buckets.push({
      start: iso(at), end: iso(until), label: capacityHourLabel(at, intervalMinutes),
  ...
  return {
    date, window, intervalMinutes, observedUntil: iso(end), isCurrent: start <= now && now < ms(window.end),
...
export function capacityCsv(report, title) {
  const fields = ['event_or_night', 'operating_date', 'interval_start_utc', 'interval_end_utc', 'interval_local', 'timezone', 'entries', 'exits', 'peak_recorded', 'interval_end_count', 'corrections', 'correction_delta', 'coverage', 'attribution', 'interval_minutes'];
  const rows = report.buckets.map((b) => [title, report.date, b.start, b.end, b.label, CAPACITY_TIMEZONE, b.entries, b.exits, b.peak, b.closing, b.corrections, b.correctionDelta, b.coverage, 'Venue-wide operating window; not event-exclusive', report.intervalMinutes || 60]);
