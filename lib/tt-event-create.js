...
// fields. Times carry the free-text values the admin entered and are only
// appended when present, matching the conservative pattern used for the start.
//
// The end DATE mirrors the start date for a same-day event, but rolls forward
// one calendar day when the times confidently describe an overnight (crosses
// midnight) range — e.g. "10:00 PM" → "12:00 AM" on 2026-07-04 ends on
// 2026-07-05. Without this, the end (midnight at the START of the day) would sit
// ~22 hours before the start, producing a negative-duration window in
// TicketTailor for exactly the late-night events we support. We only advance the
// date when BOTH times parse as simple clocks; for free-text/unparseable ends we
// keep the same-day mirror and make no risky date inference.
//
