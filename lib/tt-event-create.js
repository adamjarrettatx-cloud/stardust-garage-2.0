...
export function parseSimpleClockMinutes(value) {
...
export function validateCreatePayload(body) {
  ...
  // Ticket types are required only for the legacy TicketTailor path
  // (ticketing_mode='tickettailor' or unset). For internal / external /
  // none, ticket configuration happens on the event editor's Ticketing panel,
  // not on the create form, so an empty array is fine here.
  const skipTicketTypes = body?.ticketing_mode && body.ticketing_mode !== 'tickettailor';
  const rawTickets = Array.isArray(body?.ticket_types) ? body.ticket_types : [];
  if (!skipTicketTypes && rawTickets.length === 0) {
