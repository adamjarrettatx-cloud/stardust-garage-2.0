...
// Decide whether an end time is strictly after a start time, treating both as
// same-day clock values. Returns true when we either can't compare (one or both
// aren't simple clock strings) or when end is genuinely after start; returns
// false only when both parse cleanly AND end <= start. This deliberately fails
// open for free-text times we can't interpret, so we never reject a legitimate
// "9pm – midnight" style entry just because we couldn't parse it.
...
// publish. The start/end window is sent as TicketTailor's nested
// start_date[date] / start_date[time] and end_date[date] / end_date[time]
// fields. We mirror the start date for the end date (single-day events); the
// times carry the free-text values the admin entered. Times are only appended
// when present, matching the conservative pattern used for the start time.
//
// Returns a URLSearchParams instance (the route stringifies it).
export function buildEventSeriesBody({
  title,
  eventDate,
  eventTime,
  eventEndTime,
  ...
  // Single-day events end on the same calendar date; only the end time differs.
  params.append('end_date[date]', eventDate);
  if (eventEndTime) params.append('end_date[time]', eventEndTime);
