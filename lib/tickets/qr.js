...
//
// NOTE: qrMatrixToSvg takes the RAW STRING to encode as its first argument
// and does the matrix encoding internally. Passing a pre-encoded matrix (as
// this function used to) silently returned null \u2014 the inner
// encodeQrMatrix(text) call rejected the array with 'text must be a
// non-empty string', which was caught and produced null. Downstream that
// null was fed into React's dangerouslySetInnerHTML and rendered as the
// literal string 'null', which is what door-scanning users saw instead of
// a QR image.
