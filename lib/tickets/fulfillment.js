...
import { refundTicketOrder } from './stripe.js';
import { linkAcceptanceToOrder } from '../waiver/accept.js';

async function releaseAndRefundForVisibilityChange({ supabaseAdmin, hold, paymentIntent, chargeId, orderId }) {
  // Record the reason before release. If Stripe is temporarily unavailable, a
  // replay sees this marker and safely retries the same idempotent refund.
  if (hold.status === 'pending') {
    const { error: reasonErr } = await supabaseAdmin
      .from('ticket_holds')
      .update({ released_reason: 'visibility_changed' })
      .eq('id', hold.id);
    if (reasonErr) throw reasonErr;

    const { error: releaseErr } = await supabaseAdmin.rpc('release_ticket_hold', {
      p_hold_id: hold.id,
    });
  ...
  const refund = await refundTicketOrder({
    ...
    // Fulfillment can be denied before an order is created. In that case the
    // hold is the durable, unique checkout record; otherwise use the order.
    orderId: orderId || `visibility-change-${hold.id}`,
  ...
  if (orderId) {
    const { error: orderUpdateErr } = await supabaseAdmin
      ...
      .update({
        status: 'refunded',
        refunded_cents: Number(refund?.amount) || 0,
      })
      .eq('id', orderId);
    if (orderUpdateErr) throw orderUpdateErr;
  }
  console.warn('[ticket-fulfillment] refunded checkout after visibility change', {
    holdId: hold.id,
    paymentIntentId: paymentIntent.id,
    refundId: refund?.id,
  });
...
export async function finalizeTicketOrder({
  ...
  const chargeId =
    ...
    null;
  // A prior webhook can have inserted the idempotency row before an interrupted
  // consume. Preserve that real order ID for any required refund retry.
  let orderId = existingOrder?.id || null;

  // A visibility change can happen while a buyer is on Stripe Checkout. Never
  // issue tickets based on the stale public event state: the hold must either
  // have been created for the event's current share capability or the event
  // must still be public.
  if (hold.status === 'released' && hold.released_reason === 'visibility_changed') {
    await releaseAndRefundForVisibilityChange({ supabaseAdmin, hold, paymentIntent, chargeId, orderId });
    return { orderId: null, ticketIds: [], error: 'EVENT_VISIBILITY_CHANGED_REFUNDED' };
  }
  ...
  const accessStillValid = currentEvent?.visibility === 'public'
    || (hold.share_token && hold.share_token === currentEvent?.share_token);
  if (!accessStillValid) {
    await releaseAndRefundForVisibilityChange({ supabaseAdmin, hold, paymentIntent, chargeId, orderId });
    return { orderId: null, ticketIds: [], error: 'EVENT_VISIBILITY_CHANGED_REFUNDED' };
  }
  ...
  if (!orderId) {
    ...
    const insert = await supabaseAdmin
      ...
      .insert({
        ...
        // Tax was snapshotted on the hold when checkout started; carry it
        // through to the order for accurate reporting + potential refunds.
        tax_cents: hold.tax_cents || 0,
  ...
  const { data: holdConsumed, error: consumeErr } = await supabaseAdmin.rpc('consume_ticket_hold', {
    p_hold_id: hold.id,
  });
  if (consumeErr) throw consumeErr;
  if (!holdConsumed) {
    // The event update trigger may have released this hold while this webhook
    // was waiting to consume it. Never proceed to ticket issuance after a
    // failed consume; re-read the reason and refund the paid Checkout session.
    const { data: currentHold, error: currentHoldErr } = await supabaseAdmin
      .from('ticket_holds')
      .select('id, status, released_reason')
      .eq('id', hold.id)
      .maybeSingle();
    if (currentHoldErr) throw currentHoldErr;
    if (currentHold?.status === 'released' && currentHold.released_reason === 'visibility_changed') {
      await releaseAndRefundForVisibilityChange({
        supabaseAdmin,
    ...
    return { orderId: null, ticketIds: [], error: 'HOLD_NOT_CONSUMED' };
  }
  ...
  // Link the pre-payment waiver acceptance (scoped to hold_id) to the
  // now-persisted order_id. Immutable: this inserts an additional linkage
  // row rather than mutating the original evidence row. Best-effort — a
  // failure here does not roll back the order.
  try {
    await linkAcceptanceToOrder({ holdId: hold.id, orderId, eventId });
  } catch (e) {
    console.error('waiver_link_failed', {
      holdId: hold.id,
      orderId,
      err: e?.message,
    });
