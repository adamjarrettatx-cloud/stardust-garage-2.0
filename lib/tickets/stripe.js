...
import { stripe, findOrCreateStripeCustomer } from '../stripe/client.js';
import { stripeIdempotencyKey } from './codes.js';
...
export async function createTicketCheckoutSession({
  ...
  // Roll each line's kind up into one session-level tag so Stripe payments
  // can be filtered by 'tickets' / 'private_space' / 'mixed' at a glance.
  const productKinds = new Set(
    lineDescriptors.filter((l) => l.kind && l.kind !== 'fee').map((l) => l.kind)
  );
  const orderKind = productKinds.size === 0
    ? 'tickets'
    : productKinds.size === 1
      ? [...productKinds][0]
      : 'mixed';
      ...
      price_data: {
        ...
        product_data: {
          ...
          description: event.title
            ? `${line.kind === 'private_space' ? 'Private space rental' : (line.kind === 'fee' ? 'Booking fee' : 'Ticket')} for ${event.title}`.slice(0, 500)
            : undefined,
          // Per-line kind so Stripe dashboard rows are self-describing and
          // exports can be filtered by tickets vs private-space rentals.
          metadata: { kind: line.kind || 'tickets' },
        },
      },
    })),
    // Metadata carried on the Session AND propagated to PaymentIntent so
    // both event types on the webhook can find the hold and dashboard
    // filtering by order_kind works on either surface.
    metadata: {
