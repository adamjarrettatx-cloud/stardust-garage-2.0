...
//
// Kept free of runtime dependencies (no Supabase, no SDKs) so they're
// trivially unit-testable. Everything here is server-authoritative — the
// client can compute the same numbers for a preview, but the server never
...
// Split a refund amount into (tax portion, non-tax portion) proportional to
// how the ORIGINAL order was taxed. Used when SDG issues a partial refund:
// the state gets its cut of the returned tax back, and the rest is
// subtracted from ticket revenue.
//
// Rules:
//   * If the order had no tax collected (totalCents == 0 or taxCents == 0),
//     everything is non-tax and refundedTaxCents = 0.
//   * The tax portion is clamped so cumulative refunded tax can never exceed
//     the tax originally collected. Caller passes the outstanding tax budget
//     via `remainingTaxCents`.
//   * Rounds half-up on the tax portion so the non-tax portion picks up the
//     odd cent (buyer never sees this split; it's for internal reporting).
//
// Returns { taxPortionCents, nonTaxPortionCents } with the invariant
// taxPortionCents + nonTaxPortionCents === refundAmountCents.
export function splitRefundTax({ refundAmountCents, orderTotalCents, orderTaxCents, remainingTaxCents }) {
  const refund = Math.max(0, Math.floor(Number(refundAmountCents) || 0));
  const total = Math.max(0, Math.floor(Number(orderTotalCents) || 0));
  const tax = Math.max(0, Math.floor(Number(orderTaxCents) || 0));
  const budget = Math.max(0, Math.floor(Number(remainingTaxCents ?? tax) || 0));
  if (refund === 0 || total === 0 || tax === 0) {
    return { taxPortionCents: 0, nonTaxPortionCents: refund };
  }
  // Proportional split. Cap at the remaining tax budget so a series of partial
  // refunds can never over-return tax.
  const rawTaxPortion = Math.round((refund * tax) / total);
  const taxPortionCents = Math.min(refund, budget, rawTaxPortion);
  const nonTaxPortionCents = refund - taxPortionCents;
  return { taxPortionCents, nonTaxPortionCents };
}
...
// `entitlementPercent` is the buyer's automatic discount (member tier or live
// trial pass), resolved server-side by lib/tickets/entitlement.js. It defaults
// to 0 so every existing caller and test keeps its current behaviour.
//
// Entitlements and typed codes DO NOT STACK — see pickDiscountCents() in
// ./entitlement.js for the rule and why. `discountSource` in the return value
// says which one won, so the widget can name it and support can explain a
...
export function computeHoldSnapshot({
  ...
  let quantityTotal = 0;
  let subtotalCents = 0;
  // The slice of the subtotal a membership discount may touch (ticket lines
  // only). Equal to subtotalCents on a normal event; smaller when a rental or
  // other non-ticket product is in the cart.
  let entitlementBaseCents = 0;
  ...
  for (const sel of selections) {
    ...
    // A membership discount applies to TICKETS ONLY. Private-space rentals are
    // rows in the same ticket_products table (kind = 'private_space', managed
    // by PrivateSpacesManager), and a rental is a four-figure line item —
    // handing an Insider 60% off one because it happened to be sold on a
    // discounted event is not a benefit, it's a hole. Scoped structurally by
    // product kind rather than by an admin remembering to untick something.
    if (isEntitlementDiscountable(product)) entitlementBaseCents += lineSubtotal;
  }
  ...
  if (discountCode) {
    ...
    codeDiscountCents = applied;
  }

  // The automatic entitlement discount, and the no-stacking rule that decides
  // between it and a typed code. Both live in ./entitlement.js so the checkout
  // widget shows the same number this function charges.
  const pct = clampPercent(entitlementPercent);
  const entDiscountCents = entitlementDiscountCents(entitlementBaseCents, pct);
  const { discountCents, source: discountSource } = pickDiscountCents({
    codeCents: codeDiscountCents,
    entitlementCents: entDiscountCents,
    codeType: discountCode?.discount_type || null,
  });
  ...
  return {
    ...
    quantityTotal,
    subtotalCents,
    entitlementBaseCents,
    bookingFeeCents,
    discountCents,
    codeDiscountCents,
    entitlementDiscountCents: entDiscountCents,
    entitlementPercent: pct,
    discountSource,
    taxCents,
    taxRateBps: TEXAS_SALES_TAX_RATE_BPS,
    totalCents,
...
// Product kinds a membership discount is allowed to touch. Anything not in
// this set (today: 'private_space' rentals) is charged at full price no matter
// what percent the buyer's membership earns. Products predating the `kind`
// column are treated as tickets, matching ProductEditor's default.
export const ENTITLEMENT_DISCOUNTABLE_PRODUCT_KINDS = Object.freeze(['tickets']);

export function isEntitlementDiscountable(product) {
  const kind = product?.kind || 'tickets';
  return ENTITLEMENT_DISCOUNTABLE_PRODUCT_KINDS.includes(kind);
}
