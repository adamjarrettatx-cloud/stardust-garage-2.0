...
export function resolveEntitlementPercent(event, entitlement) {
  ...
  if (entitlement.kind === ENTITLEMENT_MEMBER) {
    ...
    // membership, distinct from a Trial SDG Pass). It is not a tier in the
    // registry, so give it the trial rate rather than letting it fall through
    // to some paid tier's policy.
    if (entitlement.planKey === 'trial') {
      return weekendMusicFixedPercent(event);
    }
    return clampPercent(paidTierPercent(event, entitlement.planKey));
  }

  return 0;
}

// The flat rate a weekend-music-only holder earns on this event.
function weekendMusicFixedPercent(event) {
  return event?.is_weekend_music_experience ? WEEKEND_MUSIC_FIXED_PERCENT : 0;
}

// Paid-tier percent, driven entirely by the tier's declared ticketDiscount
// policy in lib/membership-tiers.js.
//
// Note what is deliberately absent: the category default and the `?? 50` final
// fallback that getDiscountPercentForPlan ends in. Those were written for the
// TicketTailor code generator, where an unmapped category produced a code
// nobody was obliged to use. Here the number sets the price the buyer is
// charged, and the live category vocabulary ('internal', 'sdg_party',
// 'evening_music_residency', 'trial_resident_party', 'yoga_residency', 'other')
// has drifted well past the three names that table knows — every one of them
// would have inherited 50% off.
//
// So there is no implicit discount anywhere. A tier earns something only
// because its policy says so, or because someone set a number on this event.
export function paidTierPercent(event, planKey) {
  const policy = MEMBERSHIP_TIERS[planKey]?.ticketDiscount;
  if (!policy) return 0;

  if (policy.policy === TICKET_DISCOUNT_POLICY.NONE) return 0;

  if (policy.policy === TICKET_DISCOUNT_POLICY.WEEKEND_MUSIC_FIXED) {
    return event?.is_weekend_music_experience ? policy.percent : 0;
  }

  if (policy.policy === TICKET_DISCOUNT_POLICY.PER_EVENT) {
    const column = MEMBERSHIP_TIERS[planKey]?.discountColumn;
    const raw = column ? event?.[column] : null;
    // The legacy shared column is honoured as a fallback so events configured
    // before the per-tier columns existed keep working.
    const value = raw != null ? raw : event?.member_discount_percent;
    if (value == null) return 0;
    const cap = policy.maxPercent ?? 100;
    return Math.min(cap, clampPercent(value));
  }
