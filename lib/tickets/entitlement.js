...
import {
  CATEGORY_DISCOUNT_DEFAULTS,
  WEEKENDER_WEEKEND_MUSIC_DISCOUNT_PERCENT,
} from '../discountPercentResolver.js';
...
export function resolveEntitlementPercent(event, entitlement) {
  ...
  if (entitlement.kind === ENTITLEMENT_MEMBER) {
    ...
    // The Weekender is a weekend-music-only tier: 25% on a flagged event and
    // nothing anywhere else. They didn't buy a general discount.
    if (entitlement.planKey === 'weekender') {
      return event?.is_weekend_music_experience
        ? WEEKENDER_WEEKEND_MUSIC_DISCOUNT_PERCENT
        : 0;
    }
...
// Paid-tier percent for Builder / Insider.
//
// This deliberately does NOT reuse getDiscountPercentForPlan's final `?? 50`
// fallback. That fallback was written for the TicketTailor code generator,
// where an unmapped category produced a code nobody was obliged to use. Here
// it sets the price the buyer is actually charged, and the event category
// vocabulary has drifted well past the three names CATEGORY_DISCOUNT_DEFAULTS
// knows ('workshop', 'yoga', 'party'). Live categories also include 'other',
// 'sdg_party', 'trial_resident_party', 'evening_music_residency', 'internal'
// and 'yoga_residency' — every one of which would have inherited 50% off.
//
// So: an explicit override wins, a known category uses its default, and
// anything unrecognised earns nothing. Failing closed means a new category
// string can never quietly hand away half the door.
export function paidTierPercent(event, planKey) {
  ...
  if (event?.category && Object.hasOwn(CATEGORY_DISCOUNT_DEFAULTS, event.category)) {
    return CATEGORY_DISCOUNT_DEFAULTS[event.category];
  }
  return 0;
}
