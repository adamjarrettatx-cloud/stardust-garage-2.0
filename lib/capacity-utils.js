...
export function mapRpcError(error) {
  ...
  // PostgREST returns these (HTTP 404) when the called RPC or its underlying
  // table is missing from the schema cache — i.e. the capacity migrations have
  // not been applied to this database yet. Without this branch such failures
  // fall through to a generic 500 carrying raw PostgREST prose, which surfaces
  // to the door device as a confusing error. Map them to a clear, actionable
  // message so an un-applied migration is diagnosable instead of mysterious.
  if (sqlstate === 'PGRST202' || sqlstate === 'PGRST205' || /schema cache/i.test(raw)) {
    return { code: 'not_provisioned', message: 'Capacity counter is not set up on the server yet (database migration pending). Ask an admin to apply the capacity migrations.', httpStatus: 503 };
  }
...
// `role` is the minimum authorization the API route must confirm before it
// dispatches to the RPC. Values:
//   'admin'      — admins only (also enforced by the RPC via is_admin()).
//   'team'       — team + admin (also enforced by the RPC via is_team()).
//   'front_desk' — team + admin + front_desk. Only the +1 / -1 buttons are
//                  authorized for the front_desk role; the underlying RPCs
//                  (capacity_check_in / capacity_check_out) accept
//                  is_team() OR is_front_desk() per 20260919_front_desk_role.sql.
export const CAPACITY_OPERATIONS = {
  check_in:   { rpc: 'capacity_check_in',   role: 'front_desk', defaultSource: 'front_door' },
  check_out:  { rpc: 'capacity_check_out',  role: 'front_desk', defaultSource: 'exit_door' },
