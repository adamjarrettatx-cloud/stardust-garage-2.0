...
//
// SIGNNOW Y-ORIGIN (empirically confirmed 2026-07-22):
// SignNow's Fields API uses a TOP-LEFT origin — y increases DOWNWARD from the
// top of the page, in PDF points, 1:1 (no unit scaling). Verified against the
// live production account: a 1-page Letter PDF (612x792pt) with visible
// TOP/BOTTOM markers was uploaded, two text fields PUT at y=50 and y=700
// (top-left of page 0), then opened in a real signing session — the y=50 field
// rendered at the visual top, the y=700 field near the visual bottom (~92pt
// from the bottom edge), confirming y is measured from the top. The test
// document was deleted afterward.
//
// So stored (bottom-left) coords must be flipped to top-left ONLY for the
// SignNow fields payload; see fieldRectForSignNow below. The editor and
// business-value baking (pdf-lib) stay in the PDF-native bottom-left space and
// are unaffected. This constant is kept as a single seam: setting it back to
// 'bottom-left' disables the flip everywhere at once.
export const SIGNNOW_FIELD_Y_ORIGIN = 'top-left'; // 'bottom-left' | 'top-left'

// 'date' is a text field with date semantics: staff and signers see a date
// input, and it is baked/sent as text. It is a separate type (rather than just
// using 'text') so a template can express "this box is the date of the event"
// and the editor can render a date picker for it — which is what stops someone
// typing "next Friday" into an executed agreement.
export const FIELD_TYPES = ['text', 'date', 'checkbox', 'signature', 'initials'];
...
// SIGNNOW_FIELD_Y_ORIGIN. SignNow is top-left (confirmed), so stored y (bottom
// edge measured from the page bottom) is converted to the top edge measured
// from the page top: pageHeight - y - height. Setting the constant back to
// 'bottom-left' makes this an identity on y (single seam). Returns
// { x, y, width, height } as integers (SignNow wants whole units).
...
// SignNow's Fields API type vocabulary matches ours 1:1 for text, signature,
// initials and checkbox; 'date' is the one divergence and is mapped down to
// text. This map is the single seam for any future divergence.
const SIGNNOW_FIELD_TYPE = {
  text: 'text',
  // SignNow's Fields API has no first-class date field in the shape we PUT, so a
  // date is sent as a text field. The date-ness is enforced on our side (the
  // editor renders a date input and the value is stored ISO-ish); sending it as
  // 'text' is strictly safer than guessing at a provider type that may be
  // rejected mid-send.
  date: 'text',
  checkbox: 'checkbox',
  signature: 'signature',
