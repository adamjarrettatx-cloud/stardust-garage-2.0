...
// Parse SpotOn's "Added Time" cell (e.g. "6:08 AM", "12:00 PM") to a 0-23
// hour integer, or null when it doesn't match the expected h:mm AM/PM shape.
// Mirrors the backfill DO block in supabase/migrations/20260814_spoton_line_items.sql
// exactly, so a fresh import and the one-time backfill agree bit-for-bit.
export function parseAddedHour(value) {
  const s = String(value ?? '').trim();
...
// Build public.spoton_line_items insert rows straight from a batch's raw CSV
// rows (the verbatim SpotOn "order item list view" export), independent of
// whatever column mapping the admin chose for the ledger. This table always
// reads the fixed SpotOn header names directly, because it exists purely for
// the item-sales analytics page, not the ledger's flexible per-export mapping.
//
// Used by the PATCH confirm route for every future import; the one-time SQL
// backfill in the migration handled batches that existed before this table did.
export function buildSpotOnLineItems(rawRows = [], batchId) {
  if (!batchId) throw new Error('buildSpotOnLineItems requires a batchId');
  return rawRows.map((row) => ({
    batch_id: batchId,
    item_name: String(row['Item Name'] ?? '').trim() || '(unnamed item)',
    item_id: row['Item ID'] || null,
    menu_item_id: row['Menu Item ID'] || null,
    category: String(row['Category'] ?? '').trim() || null,
    quantity: Number(row['Quantity']) || 0,
    unit_price_cents: moneyToCents(row['Menu Item Price']),
    net_sales_cents: moneyToCents(row['Net Sales']),
    gross_sales_cents: moneyToCents(row['Gross Sales']),
    taxes_cents: moneyToCents(row['Taxes']),
    discounts_cents: moneyToCents(row['Discounts']),
    business_date: parseSpotOnDate(row['Business Date']),
    added_date: parseSpotOnDate(row['Added Date']),
    added_time: row['Added Time'] || null,
    added_hour: parseAddedHour(row['Added Time']),
    day_of_week: row['Day of Week'] || null,
    order_id: row['Order ID'] || null,
    order_number: row['Order Number'] || null,
    table_number: String(row['Table Number'] ?? '').trim() || null,
    employee_name: String(row['Employee Name'] ?? '').trim() || null,
    is_void: isYes(row['Is Void']),
    is_refund: isYes(row['Is Refund']),
    is_gift_card: isYes(row['Is Gift Card']),
  }));
}
