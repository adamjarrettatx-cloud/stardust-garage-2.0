...
export const GUESTLIST_AUDIT_ACTIONS = [
  ...
  'entry_removed',
  'checked_in',
  // Written by /portal/auth/callback when a Google sign-in re-points a
...
// DELETE /api/portal/guestlist/entries/[id] — this is the copy the UI reads.
...
// Loose email shape check. The database is the source of truth for uniqueness
// and format; this is here so the UI can grey out the Add button before a
// round trip. Matches every valid address well enough for a door list.
...
export function isValidGuestEmail(email) {
  ...
  if (!normalized || normalized.length > MAX_GUEST_EMAIL_LENGTH) return false;
...
// door, as a Map of entry id -> guest profile id (the key
// /api/admin/guest-signature/:id takes).
//
// Deliberately a separate pair of queries rather than extra columns on
// GRANT_SELECT: guest_profiles is a wider join than the grant list needs, and a
// failure here must not cost the caller its grants - the signature link simply
// doesn't render.
...
export async function loadEventGrants(admin, eventId) {
  ...
  const decorated = decorateGrants(grants, partnerProfiles);
  const signatures = await loadEntrySignatures(
    admin,
    decorated.map((grant) => grant.id),
  );

  return {
    grants: decorated.map((grant) => ({
      ...grant,
      entries: (grant.entries || []).map((entry) => ({
        ...entry,
        signature_profile_id: signatures.get(entry.id) || null,
      })),
    })),
  };
}
...
//     would get a link to /portal/guest-list they cannot sign in to. The admin
