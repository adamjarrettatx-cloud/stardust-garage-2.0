...
//
// A TicketTailor event series ID usually looks like `es_2245532` — an `es_`
// prefix followed by digits (this is what the TicketTailor API / dropdown
// returns). Older/legacy ids used an `ev_` prefix, so we accept both. We
// normalize loosely (trim) and validate strictly so a typo never gets written
// to events.tt_event_series_id, while still allowing an explicit clear/unlink.

const TT_SERIES_ID = /^e[sv]_[0-9]+$/;
...
// either a canonical `es_...`/`ev_...` string (link) or null (unlink), or
...
export function validateSeriesId(value) {
  ...
  if (!TT_SERIES_ID.test(value)) {
    return { ok: false, error: 'tt_event_series_id must look like "es_2245532"' };
  }
