...
// Valid membership tier keys — these are the internal Stripe/DB values
// (see lib/stripe-prices.js). They stay stable across marketing rename cycles;
// the customer-facing labels ('The Builder', 'The Insider', etc.) live in
// PLAN_DISPLAY over there.
export const MEMBERSHIP_TIER_KEYS = Object.freeze(['cowork', 'iykyk']);
...
export async function audienceForEvent(admin, { eventId, event }) {
  ...
  if (ev.required_membership_tier && MEMBERSHIP_TIER_KEYS.includes(ev.required_membership_tier)) {
