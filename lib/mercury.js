...
export function mercuryConfig(env = process.env) {
  ...
  return {
    configured: ready,
    // Limited production rollout: no live queueing, even if deployment
    // settings accidentally contain an enabled flag. Remove only in a
    // separately approved activation release after acceptance testing.
    enabled: mode === 'sandbox' && env.ARTIST_PAY_MERCURY_ENABLED === 'true' && ready,
    mode,
