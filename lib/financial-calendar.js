...
// (public.manual_income_entries) — an independent income source keyed by its
// own uuid, so it can never collide with or double-count a TT/local entry.
...
export function buildFinancialCalendar({ events = [], metrics = [], discovered = [], manual = [], today = new Date() } = {}) {
  ...
  const manualEntries = manual
    ...
    .map((row) => buildManualIncomeEntry(row, today));

  return [...localEntries, ...discoveredEntries, ...manualEntries]
