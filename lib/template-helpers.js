...
// Make `text` safe to draw with a StandardFont (WinAnsi/CP1252): map the common
// smart-punctuation that copy-paste introduces to ASCII, then drop any remaining
// character the font cannot encode (emoji, CJK, arrows, non-breaking hyphen,
// etc.). Without this, a single un-encodable character makes pdf-lib throw and
// the entire "Send via SignNow" fails with a generic error. Returns the cleaned
// string (possibly empty).
export function sanitizeForStandardFont(font, text) {
  if (!text) return '';
  // Fast path: the whole string already encodes.
  try { font.widthOfTextAtSize(text, 12); return text; } catch { /* fall through */ }
  ...
  for (const ch of mapped) {
    ...
    catch { /* drop characters the standard font cannot encode */ }
  }
  return out;
}
...
export async function bakeBusinessFields({ pdfBuffer, fieldLayout = [], fieldValues = {} }) {
  ...
  for (const f of fields) {
    ...
    if (!page) continue;
    const rawText = businessValueText(f, fieldValues?.[f.id]);
    if (!rawText) continue;
    const text = sanitizeForStandardFont(font, rawText);
    if (!text) continue;
