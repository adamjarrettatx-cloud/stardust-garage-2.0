...
import { dollarsToCents } from './tt-event-create.js';
import { formatMoney } from './studio-helpers.js';
...
// Total pay for a booking, in cents. Prefers an explicit hours_worked value
// (the new admin flow doesn't ask for slot times), falls back to the slot
// delta so any pre-existing booking that only has slot_start/slot_end keeps
// computing correctly. Returns null if there's no way to know the hours.
export function computeBookingAmountCents(booking) {
  ...
  if (!Number.isFinite(booking?.hourly_rate_cents)) return null;
  const hours = bookingHours(booking);
  if (hours === null) return null;
  return Math.round(booking.hourly_rate_cents * hours);
}

// Prefer the explicit hours_worked field; fall back to the slot delta for
// legacy bookings that only carry slot_start/slot_end.
export function bookingHours(booking) {
  const explicit = Number(booking?.hours_worked);
  if (Number.isFinite(explicit) && explicit > 0) return explicit;
  return hoursBetweenTimestamps(booking?.slot_start, booking?.slot_end);
}
...
export function formatBookingAmount(booking) {
  ...
  const hours = bookingHours(booking);
  if (hours === null) return formatMoney(totalCents);
  const hoursLabel = Number.isInteger(hours) ? hours : Number(hours).toFixed(1);
...
// Body: { pay_type, hourly_rate (dollars), hours (float), flat_amount (dollars),
//         slot_start?, slot_end? (ISO strings; both optional now) }
// Returns the DB-shaped payload (rate/amount already converted to cents).
//
// Slot times used to be required because we back-calculated pay from them;
// admins now enter hours directly, so the pickers are gone from the UI.
// The fields still exist on the row (and this helper still accepts them)
// so a future calendar/schedule feature can populate them without another
// schema round trip.
export function buildBookingPayload(body) {
  ...
  // Slot times are now optional. Only validate them if the caller supplies
  // them (a future scheduler UI might).
  let slotStartIso = null;
  let slotEndIso = null;
  const rawStart = String(body?.slot_start ?? '').trim();
  const rawEnd = String(body?.slot_end ?? '').trim();
  if (rawStart || rawEnd) {
    if (!rawStart || !rawEnd) {
      return { valid: false, error: 'Set both a start and end time, or leave both blank.' };
    }
  ...
  if (payType === 'hourly') {
    ...
    // Hours: either supplied explicitly, or derivable from slot times.
    const hoursRaw = body?.hours;
    if (hoursRaw !== undefined && hoursRaw !== null && String(hoursRaw).trim() !== '') {
      const hoursNum = Number(hoursRaw);
      if (!Number.isFinite(hoursNum) || hoursNum <= 0) {
        return { valid: false, error: 'Enter hours greater than 0.' };
      }
      // Round to two decimals to match the numeric(6,2) column so a
      // buyer-side“float drift” never trips the CHECK.
      payload.hours_worked = Math.round(hoursNum * 100) / 100;
    } else if (!slotStartIso) {
      return { valid: false, error: 'Enter hours (or set slot start/end) for hourly pay.' };
    }
  } else {
...
const BOOKING_SELECT = `
  id, event_id, contact_id, slot_start, slot_end, hours_worked, pay_type,
...
export async function loadEventBookings(admin, eventId) {
  const { data: bookings, error } = await admin
    ...
    .eq('event_id', eventId)
    // slot_start is optional now — order first by whether a slot exists, then
    // by slot time, then by created_at as a tiebreaker so bookings without a
    // slot still appear in a stable order.
