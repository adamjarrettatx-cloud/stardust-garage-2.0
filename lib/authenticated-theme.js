...
export const AUTH_THEME_INLINE_TOGGLE_PATHS = [
  '/bananas',
  '/bananas/analytics',
...
const INLINE_TOGGLE_PATHS = AUTH_THEME_INLINE_TOGGLE_PATHS.map((pattern) => ({
  pattern,
...
export function shouldUseInlineAuthenticatedThemeToggle(pathname) {
  if (!pathname) return false;
  return INLINE_TOGGLE_PATHS.some(({ regex }) => regex.test(pathname));
}

export function resolveAuthenticatedThemeToggleMode(pathname) {
  if (!resolveAuthenticatedThemeScope(pathname)) return 'none';
  return shouldUseInlineAuthenticatedThemeToggle(pathname) ? 'inline' : 'none';
}
