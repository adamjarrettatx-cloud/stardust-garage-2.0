...
function renderTicketRow({ ticketCode, productName, tierName, qrCid, viewUrl }) {
  const label = tierName ? `${productName} — ${tierName}` : productName;
  const qrHtml = qrCid
    ? `<img src="cid:${qrCid}" alt="Ticket QR" width="140" height="140" style="display:block;width:140px;height:140px;border:0;" />`
    : '';
  return `
    <tr>
      <td style="padding:16px 12px;border-top:1px solid #eee;">
        <table style="width:100%;border-collapse:collapse;">
          <tr>
            ...
            <td style="vertical-align:top;">
              <div style="font-size:13px;color:#666;text-transform:uppercase;letter-spacing:0.08em;">${label}</div>
              <div style="font-size:14px;color:#111;margin-top:6px;font-family:monospace;">${ticketCode}</div>
              ${viewUrl ? `<div style="margin-top:12px;"><a href="${viewUrl}" style="color:#111;text-decoration:underline;font-size:14px;">Open ticket</a></div>` : ''}
            </td>
          </tr>
        </table>
      </td>
...
export async function sendTicketConfirmation({
  ...
  const rowsHtml = rowsForRender.map(renderTicketRow).join('');
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td align="center" style="padding:24px 12px;">
      <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="background:#fff;border:1px solid #eee;">
        ...
        <tr><td style="padding:24px;">
          ...
          <div style="font-size:15px;color:#333;">${eventTitle}${eventWhen ? ` — ${eventWhen}` : ''}</div>
          ${addressHtml}
