...
// drops the invitee straight onto /partner/activate to set their name + photo.
function renderPartnerInviteHtml({ fullName, contactType, activationUrl, isContractor = false }) {
  const safeName = escapeHtml((fullName || '').split(' ')[0] || 'there');
  const role = contactType ? escapeHtml(contactType) : null;
  const roleLine = role
    ? `<p style="font-size:13px;line-height:1.6;color:rgba(255,255,255,0.55);margin:0 0 24px 0;text-align:left;">You're listed with us as: ${role}.</p>`
    : '';
...
export async function sendPartnerInvite({ email, fullName, contactType, activationUrl, isContractor = false }) {
  if (!email || !activationUrl) {
    throw new Error('sendPartnerInvite requires email and activationUrl');
  }
  return sendEmail({
    to: email,
    subject: 'Set up your Stardust Garage partner profile',
    html: renderPartnerInviteHtml({ fullName, contactType, activationUrl, isContractor }),
  });
}
