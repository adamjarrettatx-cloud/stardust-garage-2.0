...
// Guest Stardust-account welcome. Sent exactly once, the moment a visitor
// first creates a Stardust account through any of the three account-creation
// paths (ticket-checkout password signup, phone-verified trial-pass signup,
// or Google OAuth complete-profile). Idempotency is enforced upstream by
// lib/free-account/send-welcome.js against free_accounts.welcome_email_sent_at
// — this function is a pure sender and does not know about that column.
//
// The copy is deliberately anchored to what a guest account ACTUALLY does
// today: a wallet for their event tickets with a resend-to-email button, and
// a profile page where they can add a photo. It does not promise membership
// perks or gated features that aren't real for a guest account. A separate
// sendMemberWelcome runs on membership approval and carries that message.
function renderGuestAccountWelcomeHtml({ fullName }) {
  const safeName = escapeHtml((fullName || '').split(' ')[0] || 'there');
  return `<!DOCTYPE html><html><body style="margin:0;padding:24px;background:#0a0a0a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#f5f5f5;"><div style="max-width:560px;margin:0 auto;background:#141414;border-radius:14px;overflow:hidden;border:1px solid rgba(255,255,255,0.06);"><div style="padding:40px 32px 32px 32px;text-align:center;"><div style="font-size:11px;letter-spacing:0.32em;font-weight:600;color:rgba(255,255,255,0.4);margin-bottom:8px;">STARDUST</div><div style="font-size:12px;letter-spacing:0.32em;font-weight:400;color:rgba(255,255,255,0.4);margin-bottom:36px;">GARAGE</div><h1 style="font-size:28px;font-weight:800;margin:0 0 16px 0;letter-spacing:-0.02em;line-height:1.2;color:#ffffff;">${safeName}, your Stardust account is ready.</h1><p style="font-size:15px;line-height:1.65;color:rgba(255,255,255,0.75);margin:0 0 24px 0;text-align:left;">This account is how you keep everything in one place at Stardust Garage. It's free — no membership needed.</p><div style="background:#0a0a0a;border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:20px 22px;margin:0 0 24px 0;text-align:left;"><div style="font-size:11px;letter-spacing:0.14em;font-weight:600;color:#888;margin-bottom:14px;">WHAT'S IN YOUR ACCOUNT</div><div style="font-size:14px;color:#fff;font-weight:600;margin-bottom:4px;">Your tickets, always on hand</div><div style="font-size:13px;color:rgba(255,255,255,0.6);margin-bottom:14px;line-height:1.55;">Every ticket you buy from us lives in your wallet with a QR code. Pull it up at the door — or resend it to your email in one tap.</div><div style="font-size:14px;color:#fff;font-weight:600;margin-bottom:4px;">A profile our door staff recognize</div><div style="font-size:13px;color:rgba(255,255,255,0.6);margin-bottom:14px;line-height:1.55;">Add a photo to your profile so check-in is faster and smoother when you show up.</div><div style="font-size:14px;color:#fff;font-weight:600;margin-bottom:4px;">Faster checkout next time</div><div style="font-size:13px;color:rgba(255,255,255,0.6);line-height:1.55;">You're signed in for future ticket purchases — no re-entering your details every event.</div></div><p style="font-size:15px;line-height:1.65;color:rgba(255,255,255,0.75);margin:0 0 8px 0;text-align:left;">Two quick things worth doing now:</p><p style="font-size:14px;line-height:1.65;color:rgba(255,255,255,0.65);margin:0 0 24px 0;text-align:left;">1. Add a profile photo. 2. Check your ticket wallet for anything you've already bought.</p><a href="https://sdgatx.com/account/profile" style="display:inline-block;background:#ffffff;color:#0a0a0a;padding:14px 28px;border-radius:999px;font-size:11px;font-weight:600;letter-spacing:0.18em;text-decoration:none;margin-bottom:12px;">GO TO MY ACCOUNT</a><p style="font-size:12px;line-height:1.6;color:rgba(255,255,255,0.4);margin:16px 0 0 0;text-align:left;">Interested in becoming a member? <a href="https://sdgatx.com/membership" style="color:rgba(255,255,255,0.7);text-decoration:underline;">Read about membership</a> — it unlocks the venue on non-event nights, member pricing, and guest list slots.</p></div><div style="padding:18px 32px;text-align:center;border-top:1px solid rgba(255,255,255,0.05);font-size:11px;color:rgba(255,255,255,0.35);">hello@sdgatx.com</div></div></body></html>`;
}

export async function sendGuestAccountWelcome({ email, fullName }) {
  if (!email) {
    throw new Error('sendGuestAccountWelcome requires email');
  }
  return sendEmail({
    to: email,
    subject: 'Welcome to Stardust Garage — your account is ready',
    html: renderGuestAccountWelcomeHtml({ fullName }),
  });
}
