...
function renderPartnerInviteHtml({ fullName, role, contactTypeDisplay, activationUrl, isContractor = false }) {
  ...
  const intro = isContractor
    ? `We've set you up with a Stardust Garage ${roleLower} profile. Finish your profile, then select “Fill out w9” and submit your signed form. Once our team approves your W-9, you can be booked for shows. Request Pay becomes available 15 minutes after your booked set ends.`
...
// Guest invite. Sent to a named guest the moment a partner (promoter / artist)
// adds them to a guest list. Two jobs at once:
//   1. Tell the guest they're on the list and what the deal is (free vs
//      discounted), so a stranger who's never heard of us doesn't just get an
//      opaque "finish your profile" link.
//   2. Send them to /g/<token> so they can start their SDG account under this
//      email — that's the doorway into trial-pass, membership, and everything
//      downstream. The token is the credential; nothing else here is
//      authenticated.
// `addedByName` is the partner's display name ("Alora L.") — makes the ask
// feel like it's from a person the guest actually knows, not a venue system.
function renderGuestlistInviteHtml({
  guestFirstName,
  eventTitle,
  eventDate,
  eventTime,
  ...
  const safeName = escapeHtml(guestFirstName || 'there');
  const safeEvent = escapeHtml(eventTitle || 'an upcoming event');
  const whenParts = [eventDate, eventTime].filter(Boolean).map(escapeHtml).join(' \u00b7 ');
  const whenLine = whenParts ? ` on <strong style=\"color:#ffffff;\">${whenParts}</strong>` : '';
  const byLine = addedByName
    ? `<strong style=\"color:#ffffff;\">${escapeHtml(addedByName)}</strong> just added you to the guest list for <strong style=\"color:#ffffff;\">${safeEvent}</strong>${whenLine}.`
    : `You've been added to the guest list for <strong style=\"color:#ffffff;\">${safeEvent}</strong>${whenLine}.`;
  const dealLine =
    compType === 'free'
      ? `Free entry at the door for you.`
      : discountDetail
        ? `Discounted entry at the door: ${escapeHtml(discountDetail)}.`
        : `Discounted entry at the door.`;
  return `<!DOCTYPE html><html><body style=\"margin:0;padding:24px;background:#0a0a0a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#f5f5f5;\"><div style=\"max-width:560px;margin:0 auto;background:#141414;border-radius:14px;overflow:hidden;border:1px solid rgba(255,255,255,0.06);\"><div style=\"padding:40px 32px 32px 32px;text-align:center;\"><div style=\"font-size:11px;letter-spacing:0.32em;font-weight:600;color:rgba(255,255,255,0.4);margin-bottom:8px;\">STARDUST</div><div style=\"font-size:12px;letter-spacing:0.32em;font-weight:400;color:rgba(255,255,255,0.4);margin-bottom:36px;\">GARAGE</div><h1 style=\"font-size:28px;font-weight:800;margin:0 0 16px 0;letter-spacing:-0.02em;line-height:1.2;color:#ffffff;\">${safeName}, you're on the list.</h1><p style=\"font-size:15px;line-height:1.65;color:rgba(255,255,255,0.75);margin:0 0 20px 0;text-align:left;\">${byLine}</p><p style=\"font-size:13px;line-height:1.6;color:rgba(255,255,255,0.55);margin:0 0 28px 0;text-align:left;\">${dealLine}</p><p style=\"font-size:15px;line-height:1.65;color:rgba(255,255,255,0.75);margin:0 0 32px 0;text-align:left;\">Set up your Stardust Garage account below so the door has you and you can grab a Trial SDG Pass for future nights. Takes about a minute.</p><a href=\"${inviteUrl}\" style=\"display:inline-block;background:#ffffff;color:#0a0a0a;padding:14px 28px;border-radius:999px;font-size:11px;font-weight:600;letter-spacing:0.18em;text-decoration:none;margin-bottom:16px;\">CLAIM YOUR SPOT</a><p style=\"font-size:12px;line-height:1.6;color:rgba(255,255,255,0.4);margin:16px 0 0 0;\">This link is tied to your name on the list. Keep it — it's how the door confirms you.</p></div><div style=\"padding:18px 32px;text-align:center;border-top:1px solid rgba(255,255,255,0.05);font-size:11px;color:rgba(255,255,255,0.35);\">hello@sdgatx.com</div></div></body></html>`;
}
...
// Sent to the individual guest a partner just added — see
// renderGuestlistInviteHtml() above for what's in the body and why.
// `inviteUrl` must be an absolute /g/<token> URL; the token is the credential
// so nothing else authenticates the click.
export async function sendGuestlistInvite({
  email,
  guestName,
  eventTitle,
  eventDate,
  ...
  if (!email || !inviteUrl) {
    throw new Error('sendGuestlistInvite requires email and inviteUrl');
  }
  const guestFirstName = (guestName || '').trim().split(/\s+/)[0] || null;
  return sendEmail({
    to: email,
    subject: eventTitle
      ? `You're on the guest list for ${eventTitle}`
      : `You're on the Stardust Garage guest list`,
    html: renderGuestlistInviteHtml({
      guestFirstName,
      eventTitle,
      eventDate,
      eventTime,
