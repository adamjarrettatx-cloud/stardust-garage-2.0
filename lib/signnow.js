 ...
 * SignNow signs the raw request body with HMAC-SHA256 and sends it base64 in a
...
// vocabulary, or null when the event isn't a status we track. SignNow emits
// event names like "document.complete", "document.update",
// "invite.<role>.signed", etc. We also tolerate an embedded field_invites array
// (same shape getSignatureStatus reads) and defer to mapInviteStatusToContract.
export function parseWebhookContractStatus(payload) {
  if (!payload || typeof payload !== 'object') return null;

  // Prefer an explicit field_invites array when present — most reliable.
  const fieldInvites =
  ...
  // "document.complete" / "invite.complete" / "...signed" / "...fulfilled".
  if (event.includes('complete') || event.includes('signed') || event.includes('fulfill')) {
    return 'signed';
  }
  return null;
}
