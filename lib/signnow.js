...
/**
 * Add fields to an already-uploaded SignNow document. Fields can only be added
 * to a document that has NOT yet been sent to signers. Each field carries its
 * geometry (x/y/width/height/page_number), type, `role` (e.g. "Signer 1"), and
 * `required` flag. SignNow implicitly creates the named roles from these fields;
 * the subsequent invite's `to[].role` values must match them exactly.
 *
 * COORDINATE ORIGIN: the x/y are produced by buildSignNowFields() in
 * lib/contract-fields.js, which encodes the (documented-but-unverified)
 * bottom-left/points assumption behind a single flippable constant. See that
 * file's COORDINATE SYSTEM note.
 *
 * @param {string} documentId
 * @param {Array<object>} fields  SignNow field objects (see buildSignNowFields)
 */
export async function addDocumentFields(documentId, fields = []) {
  assertConfigured();
  if (!documentId) throw new Error('addDocumentFields: documentId is required');
  if (!Array.isArray(fields) || fields.length === 0) return null;
  return apiFetch(`/document/${encodeURIComponent(documentId)}`, {
    method: 'PUT',
    headers: { ...bearerHeaders(), 'Content-Type': 'application/json' },
...
/**
 * Send a fully-prepared contract for signature: upload the (already
 * business-baked) PDF, attach the signer-fillable fields, then invite the
 * signers with roles that line up with those fields.
 *
 * Roles are numbered by signing order to match the "Signer N" roles emitted by
 * buildSignNowFields: signer at order 1 -> "Signer 1", order 2 -> "Signer 2".
 * This is the field-aware counterpart to sendForSignature(); the plain
 * upload+invite path (no fields) stays untouched for backward compatibility.
 *
 * @param {object} args
 * @param {Buffer|Uint8Array} args.fileBuffer  prepared PDF bytes
 * @param {string} args.filename
 * @param {Array<{name:string,email:string,order:number,role:string}>} args.signers
 * @param {Array<object>} [args.fields]  SignNow field objects (signer fields only)
 * @param {string} [args.subject]
 * @param {string} [args.message]
 * @param {string} [args.fromEmail]
 * @returns {Promise<{ envelopeId: string, status: string }>}
 */
export async function sendContractForSignature({ fileBuffer, filename, signers = [], fields = [], subject, message, fromEmail } = {}) {
  assertConfigured();
  if (!fileBuffer || !fileBuffer.length) throw new Error('sendContractForSignature: fileBuffer is required');
  if (!Array.isArray(signers) || signers.length === 0) throw new Error('sendContractForSignature: at least one signer is required');

  // 1) Upload the prepared PDF (multipart). Returns { id }.
  const fd = new FormData();
  const bytes = fileBuffer instanceof Uint8Array ? fileBuffer : new Uint8Array(fileBuffer);
  fd.append('file', new Blob([bytes], { type: 'application/pdf' }), filename || 'contract.pdf');
  const uploaded = await apiFetch('/document', {
  ...
  // 2) Attach the signer-fillable fields (skipped when there are none).
  if (Array.isArray(fields) && fields.length) {
    await addDocumentFields(documentId, fields);
  }

  // 3) Invite with roles matching the field roles. Signers are ordered by their
  //    signing order; role name = "Signer N" (or "Approver N") by position.
  const to = signers
    .slice()
    .sort((a, b) => (a.order || 1) - (b.order || 1))
    .map((s, i) => {
      const order = s.order || i + 1;
      const roleName = s.role === 'approver' ? `Approver ${order}` : `Signer ${order}`;
      return {
        email: s.email,
        role_id: '',
        role: roleName,
  ...
  await apiFetch(`/document/${encodeURIComponent(documentId)}/invite`, {
    method: 'POST',
    headers: { ...bearerHeaders(), 'Content-Type': 'application/json' },
    body: JSON.stringify({
      to,
...
// Delete a SignNow document. Best-effort cleanup helper (e.g. removing a
// coordinate-verification test doc). Throws SignNowApiError on failure.
