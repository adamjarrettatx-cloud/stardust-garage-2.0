...
// Pure resolvers live in a sibling module so they can be unit-tested without
// dragging in the TicketTailor SDK (whose '@/...' imports node's bare test
...
export async function createCodeForMember({
  ...
  member,
  ticketTypeIds,
  discountPercent, // optional caller override; if omitted, resolve from event + member.subscription_plan
}) {
  ...
  const percent =
    discountPercent != null
      ? discountPercent
      : getDiscountPercentForPlan(event, member?.subscription_plan);

  // A 0% discount code is pointless — don't create one. This matters for
  // Weekender members on non-Weekend-Music events: getDiscountPercentForPlan
  // returns 0 (correctly, per the tier's promise), and we should quietly
  // skip rather than issue a useless code.
  if (!percent || percent <= 0) return null;

  const { id: discountId, code } = await createDiscountCode({
    code: codeString,
    name: `Member ${percent}% — ${event.title}`,
    ticketTypeIds,
    expiresUnix,
