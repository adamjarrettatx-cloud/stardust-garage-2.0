...
export async function loadEventsCalendarData(supabase) {
  ...
  // calendar_viewer is the hard-locked read-only role (see
  // 20260918_calendar_viewer_role.sql). RLS already prevents them from writing
  // team_events; this flag is what the client uses to hide every affordance
  // that would otherwise offer a create/edit modal, the CHAT/TASKS/SOPS
  // buttons, or the chat-unread badge. Server-scoped: never trust the client.
  const isCalendarViewer = teamMember.role === 'calendar_viewer';
  ...
  // Skip entirely for calendar_viewer: they have no /team/chat access and
  // the RPC would only return zero (or nothing) via its own gating.
