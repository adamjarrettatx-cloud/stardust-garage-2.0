...
//   linkingConfigs    — event_financial_config rows whose contract_id == contract.id
//   eventInputsById   — map of event_id -> boolean: does the event have any
//                       financial inputs (config/POS/TT metrics) that would be
//                       affected if the terms vanished?
//
// Returns:
//   {
...
export function assessContractDeletionImpact({
  ...
  const linkedEventId = contract.event_id || null;
  // An auto-resolve link matters only when the contract actually carries terms
  // AND that event has inputs that would change if the terms disappeared.
  const autoResolveMatters =
    hasTerms && linkedEventId && eventInputsById[linkedEventId] === true;
  if (autoResolveMatters) {
    reasons.push(
      'This contract is the auto-resolved split source for an event with financial inputs; deleting it makes that event fall back to 100% Stardust.',
    );
  }
