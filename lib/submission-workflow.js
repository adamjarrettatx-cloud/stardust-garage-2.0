export const SUBMISSION_STATUS_META = {
  ...
  // Signups-only. Signups are a passive subscriber list, so loading the admin
  // page flips new → seen instead of exposing a manual action. Deliberately
  // absent from ALLOWED_TRANSITIONS so the shared status API can never move the
  // other four submission types here.
  seen: {
    value: 'seen',
    label: 'Seen',
    tabLabel: 'Seen',
...
// changes its status. 'seen' is absent on purpose — it belongs to the signups
// page-load transition only, so statuses missing here expose no explicit
// transitions at all.
