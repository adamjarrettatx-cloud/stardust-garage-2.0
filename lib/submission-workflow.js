export const SUBMISSION_STATUS_META = {
  ...
  // For signups (a passive subscriber list) this is applied automatically on
  // page load — see lib/signups-seen.js. For the other four submission types
  // it's a manual "Mark as seen" action: an acknowledgment with no decision
  // attached yet, sitting between New and Contacted/Pending.
  seen: {
    value: 'seen',
    label: 'Seen',
    tabLabel: 'Seen',
...
export const SUBMISSION_LIST_TABS = [
  ...
  { id: 'seen', label: SUBMISSION_STATUS_META.seen.tabLabel, color: SUBMISSION_STATUS_META.seen.color },
...
export const SUBMISSION_TYPE_CONFIGS = {
  applications: {
    ...
    noun: 'application',
    tabs: ['new', 'seen', 'contacted', 'pending', 'approved', 'rejected'],
  },
  collaborations: {
    type: 'collaborations',
...
// new → seen | contacted | pending | rejected
// seen → contacted | pending | rejected
// contacted | pending → approved | rejected
// Approved and rejected are terminal, and nothing ever moves back to new:
// viewing a submission never changes its status (marking it "seen" is always
// an explicit click, never automatic, for these four types). Reject is
// reachable straight from new/seen so an admin can dismiss a submission
// immediately without first marking it contacted or pending. Approve still
// requires contacted/pending first. signups uses this same 'seen' status but
// transitions into it automatically on page load via lib/signups-seen.js,
