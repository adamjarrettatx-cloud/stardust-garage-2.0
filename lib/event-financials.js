...
// Clamp a value to >= 0 so a data anomaly (fees > gross) never shows negative.
function nonNeg(n) {
  return n < 0 ? 0 : n;
}
...
export function summarizePosRows(rows = [], { windowStart = null, windowEnd = null, salesTaxBps = 0, ccFeeBps = 0 } = {}) {
  ...
  for (const row of rows) {
    ...
    // Prefer explicit per-row tax/fee; fall back to config rates on gross.
    const hasTax = row.tax_cents != null || row.taxCents != null;
    const hasCc = row.cc_fee_cents != null || row.ccFeeCents != null;
    const tax = hasTax ? toCents(row.tax_cents ?? row.taxCents) : Math.round(gross * taxRate);
    const cc = hasCc ? toCents(row.cc_fee_cents ?? row.ccFeeCents) : Math.round(gross * ccRate);
    grossCents += gross;
    taxCents += tax;
    ccFeeCents += cc;
  }
