...
export function previewResponse(response, context, request) {
  ...
  // `no-referrer` also forces native same-origin POST forms to send
  // `Origin: null` in Chromium. Exit validates the exact sandbox Origin, so
  // retain same-origin provenance while continuing to disclose no referrer
  // information to the owner site or any other cross-origin destination.
  ...
  response.headers.set('Content-Security-Policy', [
    ...
    // Native exit POSTs redirect to this one owner page. Browsers enforce
    // form-action on redirects too; allow that destination, not outside forms.
    `form-action 'self' ${config.controllerOrigin}/bananas/view-portal`,
    // Only the isolated site may frame itself for the owner mobile viewport.
    // External sites, including production, still cannot embed preview pages.
