...
export async function loadTrialPassAnalytics({ admin: injectedAdmin } = {}) {
  ...
  const [passesRes, checkinsRes] = await Promise.all([
    admin
      .from('trial_passes')
      .select(
        'id, full_name, email, status, signup_source, issued_at, expires_at, extended_until, activated_at, signup_expires_at, applied_at, converted_at, phone_verified_at, created_by, guest_profile_id',
      )
      .order('issued_at', { ascending: false })
      .limit(2000),
    admin
      .from('trial_pass_checkins')
      .select('trial_pass_id, result, notes, checked_in_at')
