...
export async function loadTrialPassAnalytics({ admin: injectedAdmin, includePhotos = false } = {}) {
    ...
    admin
      ...
      .select(
        'id, full_name, email, phone, status, signup_source, issued_at, expires_at, extended_until, activated_at, signup_expires_at, applied_at, converted_at, phone_verified_at, created_by, guest_profile_id' +
          (includePhotos ? ', profile_photo_path' : ''),
      )
  ...
  const analytics = computeAnalytics({ passes, checkins, events });
  // CSV exports and non-visual callers do not mint or receive photo URLs.
  // The page opts in only after its existing team authorization check.
  if (includePhotos) {
    const { resolveMemberPhotoUrls } = await import('@/lib/member-photo');
    const photos = await resolveMemberPhotoUrls(admin, passes);
    for (const row of analytics.recent) row.photoUrl = photos.get(row.id) || null;
    for (const event of analytics.byEvent) {
      for (const row of event.signups) row.photoUrl = photos.get(row.passId) || null;
    }
  }
  return analytics;
}
