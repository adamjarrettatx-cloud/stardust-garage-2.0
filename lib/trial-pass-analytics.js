...
export async function loadTrialPassAnalytics({ admin: injectedAdmin } = {}) {
  ...
  // Second query for the events referenced by any check-in. We deliberately
  // fetch only the events that show up in the check-in log rather than the
  // entire calendar; the by-event tab is about attendance, not the schedule.
  const eventIds = [...new Set(checkins.map((c) => c.event_id).filter(Boolean))];
  let events = [];
  if (eventIds.length > 0) {
    ...
    if (eventsRes.error) {
      ...
      // Non-fatal: the by-event tab will label missing rows as "Unknown event".
    } else {
...
// Group every check-in by event so Adam can see which events pulled trial
// members and export the attendee list per event for follow-up.
//
// Shape:
//   [
//     {
//       eventId, title, eventDate, eventTime,
//       allowedCount,       // unique passes actually let in
//       rejectedCount,      // 'rejected' scans (photo/id mismatch, etc.)
//       deniedCount,        // 'denied_*' scans (expired/ineligible/duplicate)
//       attendees: [
//         { passId, fullName, email, phone,
//           scannedAt, result, rejectReason,
...
//   ]
//
// Sorted newest event first. Attendees within an event are sorted by scan
// time descending. Check-ins with no event_id are grouped under a synthetic
// bucket so a front-desk-only scan on a night with no ticketed event still
...
export function computeByEvent({ passes, checkins, events = [] }) {
  ...
  const groups = new Map(); // eventId (or NO_EVENT_KEY) -> group
  const NO_EVENT_KEY = '__no_event__';

  for (const c of checkins) {
    const key = c.event_id || NO_EVENT_KEY;
    let g = groups.get(key);
    if (!g) {
      const ev = c.event_id ? eventById.get(c.event_id) : null;
      g = {
        eventId: c.event_id || null,
        title: ev?.title || (c.event_id ? 'Unknown event' : 'No event (front desk)'),
        eventDate: ev?.event_date || null,
        eventTime: ev?.event_time || null,
        allowedCount: 0,
        rejectedCount: 0,
        deniedCount: 0,
        _allowedPassIds: new Set(),
        _rows: [],
      };
      groups.set(key, g);
    }
    ...
    if (c.result === 'allowed') {
      g._allowedPassIds.add(c.trial_pass_id);
    } else if (c.result === 'rejected') {
      g.rejectedCount++;
    } else if (typeof c.result === 'string' && c.result.startsWith('denied')) {
      g.deniedCount++;
    }

    // Only build attendee rows for the people who actually got in. Denied and
    // rejected scans are surfaced as counts on the event card — the drill-down
    // list is "who came to this event", which is what makes it useful for
    // follow-up outreach.
    if (c.result === 'allowed') {
      const p = passById.get(c.trial_pass_id);
      if (p) {
        g._rows.push({
          passId: p.id,
          fullName: p.full_name,
  ...
  for (const g of groups.values()) {
    // Deduplicate attendees by pass id: the one-allowed-per-event unique index
    // makes this mostly a no-op, but the older 'denied_duplicate' rows and
    // any backfilled data can produce repeats we do not want in a CSV.
    const seen = new Set();
    const attendees = [];
    ...
    attendees.sort((a, b) => new Date(b.scannedAt) - new Date(a.scannedAt));
    ...
    out.push({
      ...
      allowedCount: g._allowedPassIds.size,
  ...
  // Newest event first. Events without a date (the front-desk-only bucket)
  // fall to the bottom so real events lead the list.
  out.sort((a, b) => {
