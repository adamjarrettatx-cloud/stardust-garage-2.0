...
//
// Require first and last name. Shape validation cannot verify legal identity.
export function validateTrialPassIntake(input) {
  const name = validateLegalName(input?.fullName);
  const phoneRaw = typeof input?.phone === 'string' ? input.phone.trim() : '';
  const emailRaw = typeof input?.email === 'string' ? input.email.trim() : '';

  if (!name.valid) return { valid: false, field: 'fullName', error: name.error };
...
export const DOOR_RESULTS = {
  ...
  denied_no_photo: 'denied_no_photo',
};

// Which events a trial pass gets in to: exactly the ones flagged as a Weekend
// Music Experience, and nothing else.
//
// This used to be derived -- Friday/Saturday/Sunday AND a category matching
// /music/i -- which gave the venue TWO different definitions of "Weekend Music
// Experience" that disagreed in both directions:
//
//   * Pricing keys on the manual events.is_weekend_music_experience checkbox.
//     Tomorrow's two flagged parties are category='internal', so they were
//     priced with the 25% trial discount and then REFUSED at the door.
//   * The 9/18 and 9/19 residency nights are category='evening_music_residency'
//     (matches /music/i) but are not flagged, so a trial pass was admitted to
//     events it got no discount on.
//
// Exactly backwards, in other words. There is only one source of truth for
// whether a night is a Weekend Music Experience and it is the manual checkbox,
// so the door reads that. Pricing and door access now agree by construction
// rather than by two rules being kept in sync by hand.
//
// The weekday guard is gone with the category regex on purpose. Adding it back
// would recreate the same class of bug the moment a flagged event lands on a
// Thursday: the checkbox would grant the discount while the door refused it.
...
export function isEventTrialEligible(event) {
  ...
  // Strict true. A missing column reads as undefined, and "we forgot to select
  // it" must not silently become "everyone gets in".
  return event.is_weekend_music_experience === true;
}

// Photo-on-file check for the door decision. A trial pass that made it past
// intake without a photo (legacy row, or intake before PR #205 landed) must
// upload one before it can activate — otherwise the whole photo-verification
// door flow is worthless, since staff have nothing to compare a face to.
//
// The check is on `profile_photo_path` being a non-empty string. `uploaded_at`
// is not required — a row with a path but no timestamp is treated as "photo
// exists" because the upload endpoint sets both together, and any drift would
// be an operator's manual DB edit that should still let the guest in.
export function hasPassPhoto(pass) {
  const path = pass?.profile_photo_path;
...
export function evaluateDoorScan({ pass, event, alreadyCheckedIn = false, now = new Date() } = {}) {
  ...
  if (event && !isEventTrialEligible(event)) {
    return {
      ...
      staffAction: 'Trial passes only cover events marked as Weekend Music Experiences.',
    };
  }
  ...
  // Photo gate. Runs LAST among the denials so the guest sees the most
  // actionable message: if the pass is expired or the event is wrong, those
  // are the real block and telling them to upload a photo is useless noise.
  // But if the pass is otherwise good, missing photo is what stops activation.
  //
  // Deliberately blocks BEFORE the `allowed` branch — no photo means no
  // activation, no 30-day-clock start, no application-invite email. The row
  // stays exactly as it was so once the guest uploads on their phone and the
  // door re-scans, everything proceeds cleanly.
  if (!hasPassPhoto(pass)) {
    return {
      result: DOOR_RESULTS.denied_no_photo,
      allowed: false,
      reason: 'No photo on file for this pass.',
      staffAction: 'Ask the guest to open their pass link and upload a selfie, then re-scan.',
    };
  }
