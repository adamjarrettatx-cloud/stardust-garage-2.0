...
export async function getCurrentUser(request = null) {
  ...
  return { user, isAdmin, teamRole, isCalendarViewer };
}
...
export async function requireTeam(request = null) {
  ...
  // NOTE: calendar_viewer is deliberately NOT accepted here. That role is a
  // hard-locked read-only calendar identity (see 20260918_calendar_viewer_role.sql);
  // requireTeam() gates every write-capable team API route, so admitting
  // calendar_viewer would silently re-open exactly the surfaces we locked
  // them out of. Only 'team' and 'admin' pass.
