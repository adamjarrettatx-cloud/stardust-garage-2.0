...
export async function getCurrentUser(request = null) {
  ...
  // Source of truth: team_members.role. We use the cookie-scoped client for
  // the row read because RLS on team_members allows self-read either way and
  // there is no perf win to keep another client around.
  const supabase = await createClient();
