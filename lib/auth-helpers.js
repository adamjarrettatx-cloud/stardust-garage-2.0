...
export async function adminPageGate() {
  const { user, isAdmin } = await getCurrentUser();
  if (!user) return { user: null, isAdmin: false, redirect: '/admin/login' };
  ...
  if (adminMfaEnforced()) {
    ...
    if (!status.mfaSatisfied) {
      ...
      return { user, isAdmin: true, redirect: '/admin/security?mfa=required' };
    }
  }
...
export async function ownerPageGate() {
  ...
  if (!isOwner) return { user, isOwner: false, redirect: '/admin' };
