...
// which only /api/partner/complete-activation can set (see the column-level
...
// /partner/activate by the page, not handed guest list access.
...
// Hard gate for creating Team Chat channels. Owner-only, and deliberately a
// separate gate from requireOwner(): requireOwner() also guards whole-business
// cash flow, so widening its email set to cover the admin@sdgatx.com login
// would hand that account the financial ledger too. See canCreateChatChannel()
// in lib/chat.js for the identity rule, and the migration for the DB-level stop.
export async function requireChatChannelAdmin() {
  const { user, teamRole } = await getCurrentUser();
  if (!canCreateChatChannel({ role: teamRole, email: user?.email })) {
