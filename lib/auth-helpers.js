...
// in a security context. Middleware and sensitive routes both use the
// server-controlled team_members row.
//
// Pass `request` (the Next.js Request object) when calling from an API route
// that must also serve the mobile app — that flips resolution to the
// bearer-token path via getRequestUser(), which reads
// `Authorization: Bearer <supabase access token>` in place of a session
// cookie. The team_members lookup then runs against the resolved user id.
// Backwards-compatible: callers that don't pass `request` (every existing
// server component, every non-mobile route) keep the cookie-only behavior.
export async function getCurrentUser(request = null) {
  ...
  // Source of truth: team_members.role. When getRequestUser resolved an
  // Authorization bearer token, use that same verified token for this RLS
  // self-read; a cookie-scoped client has no mobile JWT and would otherwise
  // see no team_members row. Cookie callers keep their cookie-scoped client.
  ...
  return { user, isAdmin, teamRole, isCalendarViewer, isFrontDesk };
}
...
//
// Pass `request` when the route must also serve the mobile app (bearer token)
// — see getCurrentUser() above. Existing cookie-only callers keep working
// unchanged.
export async function requireAdmin(request = null) {
  const { user, isAdmin } = await getCurrentUser(request);
  if (!user || !isAdmin) {
...
// — see getCurrentUser() above. Existing cookie-only callers keep working
// unchanged.
export async function requireTeam(request = null) {
  const { user, isAdmin, teamRole } = await getCurrentUser(request);
  // NOTE: calendar_viewer and front_desk are deliberately NOT accepted here.
  // Both are hard-locked, single-purpose roles (see
  // 20260918_calendar_viewer_role.sql and 20260919_front_desk_role.sql);
  // requireTeam() gates every write-capable team API route, so admitting
  // either would silently re-open exactly the surfaces we locked them out
  // of. Only 'team' and 'admin' pass. Front-desk-eligible routes should use
  // requireFrontDeskOrTeam() below instead.
...
// Hard gate for the narrow set of API routes that /capacity/front-desk relies
// on. Accepts admin, team, OR front_desk — but NOT calendar_viewer, and NOT
// unauthenticated. This is the ONLY place the front_desk role is admitted
// server-side; every other require* helper continues to reject it.
//
// Use this on the routes hit by app/capacity/front-desk/* and nothing else:
//   /api/capacity/status
//   /api/capacity/operation                (check_in / check_out only —
//                                           the underlying RPC still refuses
//                                           start/end/adjust/reset from a
//                                           front-desk caller)
//   /api/capacity/checkins
//   /api/capacity/guestlist/events
//   /api/capacity/guestlist/operation
//   /api/capacity/trial-pass/scan
//   /api/door-session/active
//   /api/door-session/start
//   /api/door-session/end
//   /api/team/trial-pass/today
//   /api/team/trial-pass/manual
//   /api/tickets/scan
//   /api/scan/member-id
//
// Pass `request` when the route must also serve the mobile app (bearer token).
export async function requireFrontDeskOrTeam(request = null) {
  const { user, isAdmin, teamRole } = await getCurrentUser(request);
  if (
    !user ||
    (teamRole !== 'team' && teamRole !== 'admin' && teamRole !== 'front_desk')
  ) {
    return { user: null, isAdmin: false, teamRole: null, unauthorized: true };
  }
  return { user, isAdmin, teamRole, unauthorized: false };
...
// ---------------------------------------------------------------------------
//
// Partners (promoters, collectives, vendors — a contact with a login)
// can also hold member/free profiles under the same auth user. Partner access
// does not grant staff privileges or membership benefits.
// Source of truth is partner_profiles.is_active,
...
//
// Pass `request` when calling from an API route that must also serve the
// mobile app; resolution flips to the Authorization: Bearer path (same shape
// as getCurrentUser above) and the RPC call runs against the bearer-scoped
// client so partner_self() sees the correct auth.uid().
export async function getCurrentPartner(request = null) {
  const bearerToken = request
    ? parseBearerToken(request.headers.get('authorization'))
    : null;
  const user = request
  ...
  // Same reasoning as getCurrentUser: a cookie-scoped client cannot see the
  // mobile JWT, so partner_self() would return no row. When we have a bearer
  // token, build a bearer-scoped client for the RPC call.
  const supabase = bearerToken
    ? createSupabaseClient(
        process.env.NEXT_PUBLIC_SUPABASE_URL,
        process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY,
  ...
  const [{ data, error }, { data: staff, error: staffError }] = await Promise.all([
  ...
  return {
    user,
    partner,
    isActivePartner: !error && !staffError && !isRestrictedStaffRole(staff?.role)
      && partner?.is_active === true,
  };
}

/**
 * Returns a Supabase client scoped to whichever auth material this request
 * carries — Authorization: Bearer <jwt> when the mobile app is calling, or
 * the session cookies otherwise. Use this inside a POST handler that must
 * write through RLS as the calling user (e.g. an INSERT that a
 * partner-owns-grant policy will re-verify): a bare `await createClient()`
 * only reads cookies and would 401 for a mobile caller even when their JWT
 * is valid.
 */
export async function createRequestScopedClient(request) {
  const bearerToken = request ? parseBearerToken(request.headers.get('authorization')) : null;
  if (bearerToken) {
...
//
// Pass `request` from a route handler that must also serve the mobile app so
// the check runs against the Authorization: Bearer token instead of session
// cookies. Legacy server-component callers keep the parameterless behavior.
export async function requirePartner(request = null) {
  const { user, partner, isActivePartner } = await getCurrentPartner(request);
