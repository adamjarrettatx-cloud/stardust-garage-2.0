...
// contract_id if set, else the most recent signed contract linked to the
// event, else the most recent contract linked to the event.
async function resolveContract(admin, eventId, configContractId) {
  ...
  // Prefer a fully-signed contract if present.
  return linked.find((c) => c.status === 'signed') || linked[0];
}
