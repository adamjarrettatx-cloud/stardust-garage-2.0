...
export function normalizeCachedMetrics(row) {
  ...
  // Prefer the stored net, but fall back to gross - fees if it's missing. Clamp
  // the derived value at 0 so a fees>gross data anomaly never shows negative net.
  const netCents = row.net_cents == null ? Math.max(0, grossCents - feesCents) : toCents(row.net_cents);
  return {
    ...
    // `refreshed` is a successful TT pull (even if it found zero sales), so the
    // UI can show a real $0.00 instead of "—". `hasData` is the stronger flag
    // for portfolio totals: a refresh that actually produced sales/revenue.
    refreshed: row.status === 'ok',
...
// Zero-valued defaults for the NOT NULL numeric/jsonb columns of
// public.event_ticket_metrics. Every cache row — ok, not_configured, or error
// — MUST carry these keys so a single batched upsert builds a uniform column
// union. PostgREST fills a key that is missing from *some* (but not all) rows
// with SQL NULL, not the column DEFAULT, which would violate NOT NULL.
const EMPTY_METRICS_FIELDS = {
  tickets_sold: 0,
  orders_count: 0,
  gross_cents: 0,
  fees_cents: 0,
...
// Build a cache row for events using first-party (internal) ticketing. Same
// shape as buildMetricsSnapshot so the upsert path and the UI reader do not
// have to branch on provider. Pure: caller supplies already-fetched orders
// and tickets (see /api/admin/refresh-event-metrics).
//
// Money math mirrors /api/admin/tickets/summary/route.js so the row on the
// Events list and the roll-up on the event dashboard never disagree:
//   * gross_cents  — sum of total_cents on paid-like orders
//                    (status in paid, refunded, partial_refund)
//   * fees_cents   — sum of fees_cents on paid-like orders (booking fee we
//                    added on top; Stripe processor fees are separate and
//                    settled nightly outside this cache)
//   * net_cents    — gross − refunded, so a partial refund reduces the
//                    number the owner sees in real time
//   * tickets_sold — valid + used tickets (voided/refunded excluded), which
//                    matches how attendance is counted downstream
export function buildInternalMetricsSnapshot({
  eventId,
  orders = [],
  tickets = [],
  fetchedAt = null,
  ...
  const paidLike = orders.filter(
    (o) => o && ['paid', 'refunded', 'partial_refund'].includes(o.status),
  );
  const grossCents = paidLike.reduce((s, o) => s + toCents(o.total_cents), 0);
  const feesCents = paidLike.reduce((s, o) => s + toCents(o.fees_cents), 0);
  const refundedCents = orders.reduce((s, o) => s + toCents(o.refunded_cents), 0);
  const netCents = Math.max(0, grossCents - refundedCents);
  const ticketsSold = tickets.filter(
    (t) => t && (t.status === 'valid' || t.status === 'used'),
  ).length;
...
// Build a non-ok cache row (not_configured / error) with the SAME column set as
// buildMetricsSnapshot, so mixed batches upsert without NOT NULL violations.
// The numeric columns carry 0 and raw_summary is {} — `hasData` in
// normalizeCachedMetrics still gates these out of the displayed figures.
export function buildPlaceholderMetricsRow({
  eventId,
