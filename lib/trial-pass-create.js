...
export async function issueTrialPass({
  ...
  createdBy = null,
  createAccount = false,
  // When true, an existing pass for this email/phone is never reissued. Used
  // by the temporary SMS-bypass path: without phone verification, rotating a
  // matched pass's token would let anyone who types a stranger's email or
  // phone take over that stranger's pass.
  refuseExisting = false,
} = {}) {
  const name = validateLegalName(data?.full_name);
  if (!name.valid) return { ok: false, error: name.error, field: 'fullName', status: 400 };
  data = { ...data, full_name: name.fullName };
  ...
  if (existing && refuseExisting) {
    return {
      ok: false,
      error: 'You already have a pass — check your email for your pass link, or see the front desk.',
      status: 409,
    };
  }
