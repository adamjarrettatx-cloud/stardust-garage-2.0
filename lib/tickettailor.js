...
// flow). These POST to TicketTailor and so are only ever called from
// admin-gated server routes after requireAdminMfa().
// ---------------------------------------------------------------------------

// Creates an event series. `body` is a URLSearchParams built by
// lib/tt-event-create.buildEventSeriesBody (series-level metadata only — the
// series carries no date/time; see createEventOccurrence). Returns the created
...
// Creates an event occurrence under a series — this is what actually carries the
// event's date/time. `body` is a URLSearchParams built by
// lib/tt-event-create.buildOccurrenceBody (start_date/start_time/end_date/
// end_time). Returns the created occurrence object.
export async function createEventOccurrence(eventSeriesId, body) {
  return ttFetch(`/event_series/${eventSeriesId}/events`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: typeof body === 'string' ? body : body.toString(),
  });
...
// Sets the status of an event series. TicketTailor exposes status changes on a
// dedicated sub-resource, POST /v1/event_series/:id/status, with an UPPERCASE
// enum: 'DRAFT', 'PUBLISHED', or 'CLOSE_SALES'. We accept the common lowercase
// aliases and normalize. Returns the updated series object.
const TT_STATUS = { draft: 'DRAFT', published: 'PUBLISHED', closed: 'CLOSE_SALES', close_sales: 'CLOSE_SALES' };

export async function setEventSeriesStatus(eventSeriesId, status) {
  const normalized = TT_STATUS[String(status).toLowerCase()] || String(status).toUpperCase();
  const params = new URLSearchParams();
  params.append('status', normalized);
  return ttFetch(`/event_series/${eventSeriesId}/status`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: params.toString(),
  });
...
// All event occurrences, optionally filtered to a single series. READ ONLY.
// TicketTailor's GET /v1/events returns one object per occurrence; each carries
// its own `id` (ev_...), the parent `event_series_id` (es_...), a `name`, and
// nested `start_date`/`end_date` objects ({ date, time, iso, unix, timezone }).
// We return the raw rows and let lib/tt-discovered-events.js normalize them, so
// this stays a thin, convention-matching wrapper over ttList's cursor paging.
export async function listEvents({ eventSeriesId } = {}) {
  const params = {};
  if (eventSeriesId) params.event_series_id = eventSeriesId;
  return ttList('/events', params);
}
