...
// Creates a single-use 60% discount code restricted to the given ticket types.
// Returns { id, code }.
export async function createDiscountCode({ code, name, ticketTypeIds, expiresUnix }) {
  // TicketTailor expects form-encoded bodies and repeats array keys.
  const params = new URLSearchParams();
  params.append('code', code);
  params.append('name', name);
  params.append('type', 'percentage');
  params.append('price_percent', '60');
