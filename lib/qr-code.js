...
// auto-selecting the smallest QR version (1..40) that fits the data. That covers
// our setup URLs (~60-90 chars) with margin while keeping this file compact.
//
...
// Each entry: total data codewords, ec codewords per block, and block group
// layout [ [numBlocks, dataCodewordsPerBlock], ... ]. Covers versions 1..10,
// which hold up to ~287 byte-mode chars at level M — far beyond our URLs.
const VERSIONS_M = {
...
function buildCodewords(bytes, version) {
  ...
  const lenBits = version <= 9 ? 8 : 16; // char-count indicator length, byte mode
...
// Alignment-pattern centers per version (M level uses the same table as all).
const ALIGN_CENTERS = {
...
// Pick the smallest supported version whose data capacity holds the payload.
