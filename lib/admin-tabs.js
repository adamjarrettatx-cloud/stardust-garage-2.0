...
// section that is really a link to a page of its own — see `href` on Chat —
// has nothing to render there, so ?tab=chat resolves to the default rather
// than printing a section heading over an empty grid.
...
// one, otherwise the dashboard root with the section selected.
...
export const ADMIN_TILES = {
  ...
  chat: [],
  // The calendar used to be a tile here, under a name that implied it was
  ...
  // Calendar rather than being something you navigate to out of Team.
  // /team/calendar still exists for non-admin team members (who have no admin
  // dashboard) and sends an admin on to Events.
  //
  // Team Chat was a tile here too. It is now its own OPERATIONS section
  // directly below Events, so /team/chat is reached from the sidebar. What is
  // left in Team is the work itself.
  team: [
