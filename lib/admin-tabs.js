...
// The dashboard root can only render a tile grid or a list-backed section, so
// the default can never be a section that is really a link to a page of its own
// (see `href` on Tasks and Chat) — resolveRootAdminTab would bounce it straight
// back here and loop. Events is the landing because it is the one section that
// renders something useful with no further click: the programming calendar.
export const DEFAULT_ADMIN_TAB = 'events';
...
export const ADMIN_TABS = [
  // Tasks is a single-destination section, like Chat: exactly one page sits
  // behind it (/team/progress), so `href` sends the sidebar straight there
  // instead of to a tile grid holding one tile. It was the last tile in the old
  // Team section; once the calendar moved to Events, Chat became a section and
  // Team Members moved to Settings, Team had nothing else under it and was
  // removed rather than left as a one-tile wrapper around this page.
  {
    id: 'tasks',
    label: 'Tasks',
    group: 'OPERATIONS',
    ...
    href: '/team/progress',
    description: 'Assigned work by department.',
  },
  ...
  // List used to be a People tile, which is where its count came from; now that
  // it is only opened from a specific event row, no queued-work count belongs
  // here. Artist Pay's pending-pay-request badge follows the Artist Pay tab
  // under MONEY (see below) rather than shadowing it on Events too.
  {
    id: 'events',
    label: 'Events',
    group: 'OPERATIONS',
    ownerOnly: false,
    rendersOwnContent: true,
    // Not rendered on screen: sections that render their own content get no
    // heading block (see AdminDashboardClient). Kept because every section
    // carries a description and it is what the sidebar entry means.
    description: 'The programming calendar and every event, upcoming and past.',
  },
  // Chat is a single-destination section: there is exactly one page behind it
  // (/team/chat), so a tile grid holding one tile would only add a click. `href`
  // tells the sidebar to link straight to the page instead of to a tile grid,
  // and `rendersOwnContent` keeps the empty tile set deliberate for the
  // tile-grid contract test. The unread badge follows the section, the way the
  // pending-pay-request badge follows Events.
  {
    id: 'chat',
    label: 'Chat',
    group: 'OPERATIONS',
    ...
    rendersOwnContent: true,
    href: '/team/chat',
    countKeys: ['unreadChat'],
    description: 'Internal channels and direct messages.',
  },
  {
    id: 'memberships',
    label: 'Memberships',
  ...
  },
  {
    ...
    group: 'OPERATIONS',
    ownerOnly: false,
    description: 'Collaborations, signups, venue inquiries, and bookings.',
  },
  {
    id: 'analytics',
    label: 'Analytics',
  ...
  // Artist Pay is a single-destination section, like Chat and Tasks: exactly
  // one page sits behind it (/bananas/pay-requests), so `href` sends the
  // sidebar straight there instead of to a tile grid holding one tile. It used
  // to be reachable only from the per-event ARTIST PAY button on the Events
  // list, but the owner asked for it here so the whole queue is one click from
  // anywhere. `countKeys` keeps the pending-pay-request badge visible on the
  // sidebar the way it used to sit beside Events.
  {
    id: 'artist-pay',
    label: 'Artist Pay',
    group: 'MONEY',
  ...
  // The door workstation is a direct destination, not another tile grid.
  // Keep its existing /capacity route and role gates unchanged.
  {
    id: 'front-desk',
    label: 'Front Desk',
    group: 'ADMIN',
    ownerOnly: false,
    rendersOwnContent: true,
    href: '/capacity/front-desk',
    description: 'Guest check-in, QR scanning, and trial passes.',
  },
  {
    id: 'orders',
    ...
    rendersOwnContent: true,
    href: '/bananas/orders',
    description: 'Search ticket orders and review refunds across every event.',
  },
  {
    id: 'event-capacity',
    ...
    ownerOnly: false,
    rendersOwnContent: true,
    href: '/bananas/capacity',
    description: 'Historical and live venue capacity by event night.',
  },
  {
    id: 'contacts',
    label: 'Contacts',
    ...
    description: 'The people, collectives, renters and vendors SDG works with.',
  },
  {
    id: 'documents',
    label: 'Documents',
  ...
  },
  {
    ...
    ownerOnly: true,
    rendersOwnContent: true,
    href: '/bananas/view-portal',
    description: 'Preview the website as each account type in the isolated environment.',
  },
  {
    id: 'settings',
    label: 'Settings',
...
export function resolveAdminTab(requested, { isOwner = false } = {}) {
  const rawId = Array.isArray(requested) ? requested[0] : requested;
  // Preserve bookmarks to the two sections now combined under Incoming.
  const id = rawId === 'people' || rawId === 'rentals' ? 'incoming' : rawId;
...
// The dashboard root can only ever show a tile grid (or the Events list). A
// section that is really a link to a page of its own — see `href` on Tasks and
// Chat — has nothing to render there, so ?tab=tasks resolves to the default
// rather than printing a section heading over an empty grid.
export function resolveRootAdminTab(requested, options = {}) {
  const id = resolveAdminTab(requested, options);
  return adminTabById(id)?.href ? DEFAULT_ADMIN_TAB : id;
}

// Where the sidebar should send you for a section: its own page when it has
// one (Tasks, Chat), otherwise the dashboard root with the section selected.
export function adminTabHref(tab) {
  return tab?.href || `/bananas?tab=${tab?.id}`;
}
...
export const ADMIN_TILES = {
  ...
  events: [],
  // Chat has no tiles either: the section links straight to /team/chat.
  chat: [],
  // Same for Tasks — the section links straight to /team/progress.
  tasks: [],
  // Artist Pay is single-destination like Chat and Tasks: the sidebar links
  // straight to /bananas/pay-requests, so no tile grid.
  'artist-pay': [],
  'view-portal': [],
  orders: [],
  // Front Desk opens the existing workstation in one click.
  'front-desk': [],
  'event-capacity': [],
  contacts: [],
  // There is deliberately no `team` key. That section held four tiles at its
  // peak — the calendar (now the Events Calendar at the top of Events), chat
  // and tasks (each now a section of its own) and the team roster (now in
  // Settings). With every one of them rehoused there was nothing left for the
  // section to contain, so it was deleted rather than kept as an empty heading
  // in the sidebar.
  memberships: [
    ...
    // Manual issuance lives only in the Front Desk workstation.
  ],
  ...
  analytics: [
    // Membership Journey is the single owner-only view of the full funnel —
    // account created → trial pass → visited → applied → approved → active
    // paying member. It lives under Analytics (not Memberships) because it's
    // a cross-stage read of the whole pipeline, not a queue of per-application
    // work; the Applications and Members tiles in the Memberships tab remain
    // the drill-downs for individual decisions.
    { href: '/bananas/membership', action: 'VIEW', title: 'Membership Journey', sub: 'Account → trial → applied → active', restricted: 'owner' },
    // Sales is the newest tile in the MONEY tab — the single dashboard for the
    // first-party payment flow (internal ticket orders, memberships, refunds/
    // comps, and live Stripe payouts). Financials remains the historical income
    // accounting surface (TicketTailor + POS + manual income, bucketed by event
    // date), which is a different question. Both stay owner-only.
    { href: '/bananas/sales', action: 'VIEW', title: 'Sales', sub: 'Orders, memberships and payouts', restricted: 'owner' },
    ...
    { href: '/bananas/tax-report', action: 'VIEW', title: 'Tax Report', sub: 'Texas sales tax owed', restricted: 'owner' },
...
//
// Empty for now. Historically the two trial pass pages
// (/team/trial-pass/analytics and /team/trial-pass/manual) lived here: they
// gained tiles in the Memberships tab (#115) at the same time the shell
// learned to wrap tiled /team routes (#116), and their hardcoded dark chrome
// — a second AuthenticatedPageHeader and a <main> with #0a0a0a background —
// doubled up when placed inside the shell. Both pages have since been ported
// onto the theme tokens. Analytics remains inside the shell; the retired
// manual-issuance page now redirects to the Front Desk workstation.
//
// Keep the list and the helper: any future page that needs a temporary
// standalone period between gaining a tile and being ported to the theme
// tokens has an obvious place to opt out.
export const SHELL_EXEMPT_PREFIXES = [];
...
export const ADMIN_SECTION_ROUTES = [
  { prefix: '/bananas/view-portal', tabId: 'view-portal' },
  { prefix: '/bananas/capacity', tabId: 'event-capacity' },
  ...
  // Artist Pay is a MONEY section of its own now, so the highlight follows
  // that tab rather than Events. No `title` because the section IS the page —
  // an "Artist Pay / Artist Pay" breadcrumb would say nothing.
  { prefix: '/bananas/pay-requests', tabId: 'artist-pay' },
  { prefix: '/bananas/orders', tabId: 'orders' },
  { prefix: '/bananas/contacts', tabId: 'contacts' },
  // Chat lost its tile when it became a section, so this is what keeps the
  // sidebar highlighting Chat while you are reading messages. No `title`: the
  // page IS the section, so a "Chat / Chat" breadcrumb would say nothing.
  { prefix: '/team/chat', tabId: 'chat' },
  // Tasks the same: it stopped being a tile when it became a section, so this
  // is what keeps the sidebar highlighting Tasks while you are on the page.
  { prefix: '/team/progress', tabId: 'tasks' },
];
