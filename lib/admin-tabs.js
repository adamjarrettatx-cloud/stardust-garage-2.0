...
export const ADMIN_TILES = {
  ...
  // in the sidebar.
  memberships: [
    // The Journey hub is the single front door for the whole membership
    // funnel — trial pass → applied → approved → active paying member — in
    // one view. The other tiles below remain because they are still the
    // per-stage drill-downs; the hub deliberately doesn't replace them.
    // Owner-only, gated by `ownerPageGate` on the page itself.
    { href: '/bananas/membership', action: 'VIEW', title: 'Membership Journey', sub: 'Trial → applied → active, one view', restricted: 'owner' },
