...
// ---------------------------------------------------------------------------
// Business-week helpers (Monday start, Sunday end) for the "Done this week"
// KPI — as opposed to `completedRecently`'s rolling trailing-N-days window.
// `dateStr` is `YYYY-MM-DD`. Uses UTC-midnight day math throughout, matching
// every other date helper in this file, so results never shift with DST.
// ---------------------------------------------------------------------------
...
// True when the task is done AND was completed within the current business
// week (Monday through Sunday, inclusive) relative to `todayStr`. Resets to
// zero every Monday regardless of how many were completed the prior week.
export function completedThisWeek(task, todayStr) {
  if (task.status !== 'done') return false;
  const completed = toDateString(task.completed_at);
  if (!completed) return false;
  const monday = startOfWeekMonday(todayStr);
  const sunday = endOfWeekSunday(todayStr);
  if (!monday || !sunday) return false;
  return completed >= monday && completed <= sunday;
}

// ---------------------------------------------------------------------------
// KPI summary for the owner/admin dashboard. Operates on whatever set the
// caller passes in.
// ---------------------------------------------------------------------------
export function computeKpis(tasks, todayStr) {
  ...
  for (const task of tasks || []) {
    ...
    if (completedThisWeek(task, todayStr)) kpis.doneThisWeek += 1;
  }
  return kpis;
}
