...
import { contentDisposition } from '@/lib/document-access';
import crypto from 'node:crypto';
import { taxDocumentAccess, nonTaxStorageCheck } from '@/lib/w9/server';
import {
...
/**
 * Resolve a document version and stream its bytes back through the server,
 * logging a view (inline) or download in the audit trail.
 *
 * SECURITY: files are never exposed via a public storage URL — the service-role
 * download happens here and the signed URL never leaves the server. This helper
 * does NOT authorize anything; the caller must have already established that
 * this actor may read this document.
 *
 * @returns {Promise<Response>} the file stream, or a JSON error response.
 */
export async function streamDocumentVersion({
  admin,
  documentId,
  versionId = null,
  inline = false,
  ...
  if (!actor.id || !await taxDocumentAccess(admin, actor.id, documentId)) {
    return Response.json({ error: 'Document access denied.' }, { status: 403 });
  }
  const { data: taxSubmission, error: taxLookupError } = await admin.from('w9_submissions')
    .select('id,contact_id').eq('document_id', documentId).maybeSingle();
  const { data: document } = await admin.from('documents').select('category').eq('id', documentId).maybeSingle();
  if (document?.category === 'tax' && taxLookupError) {
    return Response.json({ error: 'Tax document access is temporarily unavailable.' }, { status: 503 });
  }
  if (taxSubmission) {
    const { error: auditError } = await admin.from('w9_audit_log').insert({
      submission_id: taxSubmission.id, contact_id: taxSubmission.contact_id,
      actor_id: actor.id, action: inline ? 'viewed' : 'downloaded',
    });
    if (auditError) return Response.json({ error: 'Could not record secure document access.' }, { status: 503 });
  }
  let q = admin.from('document_versions').select('*').eq('document_id', documentId);
  if (versionId && UUID.test(versionId)) q = q.eq('id', versionId);
  ...
  if (document?.category !== 'tax') {
    const taxError = await nonTaxStorageCheck(admin, ver.storage_path);
    if (taxError) return taxError;
  }

  const { data: blob, error } = await admin.storage.from(DOCUMENT_BUCKET).download(ver.storage_path);
  if (error || !blob) return Response.json({ error: 'Storage error' }, { status: 500 });
  ...
  return new Response(blob.stream(), {
    headers: {
      ...
      'Content-Disposition': contentDisposition(ver.filename, inline),
