...
//
// This helper is advisory (it drives the UI and the API routes' 401). The hard
// stops are the restrictive RLS policies in
// supabase/migrations/20260802_team_chat_channel_admin_and_unread.sql (creating
// channels) and 20260829_chat_owner_only_moderation.sql (deleting messages,
// channels and memberships), which a direct PostgREST call cannot get around.
export const CHAT_CHANNEL_ADMIN_EMAILS = ['admin@sdgatx.com', 'adam@sdgatx.com'];

// The one authority over chat. Named for what it now covers: creating group
// channels AND deleting anything — any message in any channel, any message in a
// DM the owner is part of, and whole channels. A team member cannot delete even
// their own message; that was the owner's explicit call, so there is no
// per-sender exception anywhere in this file.
export function canModerateChat({ role, email } = {}) {
  if (role !== 'admin') return false;
  return CHAT_CHANNEL_ADMIN_EMAILS.includes(String(email || '').trim().toLowerCase());
}

// Channel creation is one of the powers above, not a separate rule. Kept as its
// own name because that is what the sidebar's "+ NEW" button is asking about,
// and so the intent at each call site stays readable.
export function canCreateChatChannel(actor) {
  return canModerateChat(actor);
}

// Deleting a channel takes its entire message history with it (the FKs cascade),
// so it cannot be a one-click action sitting next to the channel the owner is
// reading. Both the sidebar and DELETE /api/team/chat/channels/[id] require the
// channel's name typed back, which makes a mis-click impossible and a mistaken
// delete require actually reading which channel is on screen.
//
// Matching is forgiving about the things that are display, not identity: a
// leading '#', surrounding space, and letter case. It is strict about
// everything else.
export function channelDeleteConfirmed(typed, channelName) {
  const normalize = (v) => String(v ?? '').trim().replace(/^#+/, '').trim().toLowerCase();
  const target = normalize(channelName);
  if (!target) return false;
  return normalize(typed) === target;
}
...
// Read actual clipboard files, not HTML image tags or remote URLs. Some
// browsers expose images through items, others through files. The composer
// supports one attachment, just like the existing file picker.
export function chatImageFromClipboard(clipboardData) {
