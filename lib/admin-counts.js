...
export async function fetchAdminCounts(supabase) {
  ...
  const [
    ...
    // Unread @mentions of the caller, counted separately from ordinary unread
    // messages: being named is a different, higher-priority signal than a busy
    // channel, and the chat UI surfaces the two with different markers. This is
    // a subset of unreadChat, so the two must never be summed into one badge.
  ...
  return {
    ...
    unreadChatMentions: totalUnreadCount(unreadChatMentions?.data),
