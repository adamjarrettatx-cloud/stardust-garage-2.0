...
// POST /api/portal/request-signin-link
// Body: { email: string }
//
...
export async function POST(request) {
  ...
  try {
    ...
    // This flow is a re-send: we don't have the contact type in scope here and
    // it isn't worth an extra join for a fallback email. The email falls back
    // to "portal profile" copy when role/contactTypeDisplay are omitted.
    await sendPartnerInvite({
      email,
      fullName: profile.full_name,
      // /portal/activate handles both states: it sends an already-activated
