...
export async function POST(request, { params }) {
  ...
  if (action === 'refund') {
    // All refunds must use a reviewed, durable request. Keep older clients
    // from bypassing the new per-order serialization and retry safeguards.
    return NextResponse.json({
      error: 'Use Attendees & Orders to review and confirm this refund.',
      url: `/bananas/events/${order.event_id}/attendees?order=${order.id}`,
    }, { status: 409 });
  }
