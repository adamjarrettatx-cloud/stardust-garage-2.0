  ...
  return (
    <main className="max-w-[700px] mx-auto px-6 py-16">
      ...
      <p className="text-[14px] mb-10" style={{ color: 'var(--text-3)' }}>
        Pick a billing period for your <span style={{ color: 'var(--text-1)' }}>{PLAN_DISPLAY[plan]}</span> membership. You can cancel anytime by emailing us.
      </p>

      {/* Plan switcher (only if they need to choose) */}
      {!initialPlan && (
        <div className="mb-8">
          <div className="text-[11px] font-semibold tracking-[0.18em] mb-3" style={{ color: 'var(--text-3)' }}>
            PLAN
          </div>
            ...
            {Object.entries(PLAN_DISPLAY).map(([key, label]) => (
              <button
                ...
                style={{
                  ...
                  color: plan === key ? '#0a0a0a' : 'var(--text-1)',
        ...
        {Object.entries(periods).map(([periodKey, info]) => {
          ...
          return (
            <button
              ...
              style={{
                background: isSelected ? 'var(--st-tint-amber-2)' : 'var(--surface-1)',
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  ...
                  style={{
                    borderColor: isSelected ? 'var(--st-ffb84d)' : 'rgba(255,255,255,0.3)',
                  }}
                >
                  {isSelected && (
                    <div className="w-2.5 h-2.5 rounded-full" style={{ background: 'var(--st-ffb84d)' }} />
                  )}
                </div>
      ...
      <p className="text-[11px] text-center mt-4" style={{ color: 'var(--fg-a4)' }}>
