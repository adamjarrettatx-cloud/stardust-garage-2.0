  ...
  return (
    <div className="relative group">
      <Link href={`/events/${event.slug}`} className="block">
        <div className="relative overflow-hidden rounded-[14px] bg-[var(--surface-4)] transition-transform group-hover:-translate-y-1" style={{ aspectRatio: '3 / 4' }}>
          {event.image_url && (
            <img src={event.image_url} alt={event.title} className="w-full h-full object-cover" />
          )}
        </div>
        <div className="text-xs mt-4 mb-2" style={{ color: 'var(--text-3)' }}>
