...
// lib/admin-tabs.js), so there is no tile grid for it and this list is the
// section. Which section is showing is client state owned by AdminShell and
// mirrored in ?tab=, so the check has to happen on the client — the server can
// read the query param but not a later in-page section switch, which the shell
// does with pushState rather than a navigation.
export default function EventsTabPanel({ upcoming, past }) {
