...
export default function ManualIncomeDialog({ open, editing, defaultDate, linkedEvent, onClose }) {
  ...
  const inputStyle = { background: 'var(--surface-3)', border: '1px solid var(--fg-a12)', color: 'var(--text-1)' };
  const fieldErr = (k) => fieldErrors[k] && (
    <p className="text-[11px] mt-1" style={{ color: 'var(--st-f87171)' }}>{fieldErrors[k]}</p>
  );
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <button type="button" onClick={() => !busy && onClose()} aria-label="Close" className="text-[20px] leading-none transition-opacity hover:opacity-50" style={{ color: 'var(--text-3)' }}>×</button>
        </div>

        <form onSubmit={onSubmit} className="space-y-3" noValidate>
          {localEventId && (
            <div className="text-[11px] rounded-[8px] px-3 py-2 flex items-center gap-2" style={{ background: 'rgba(74,222,128,0.1)', color: 'var(--st-4ade80)' }} data-testid="fc-manual-linked-banner">
              <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: 'var(--st-4ade80)' }} />
              <span className="truncate">Linked to event{linkedTitle ? `: ${linkedTitle}` : ''}</span>
            </div>
          )}
          <div>
            <label htmlFor="mi-date" className="block text-[11px] uppercase tracking-[0.1em] mb-1" style={{ color: 'var(--text-3)' }}>Date *</label>
            <input
          ...
          <div>
            <label htmlFor="mi-title" className="block text-[11px] uppercase tracking-[0.1em] mb-1" style={{ color: 'var(--text-3)' }}>Title / name *</label>
            <input
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="mi-amount" className="block text-[11px] uppercase tracking-[0.1em] mb-1" style={{ color: 'var(--text-3)' }}>Amount (USD) *</label>
              <input
            ...
            <div>
              <label htmlFor="mi-category" className="block text-[11px] uppercase tracking-[0.1em] mb-1" style={{ color: 'var(--text-3)' }}>Category *</label>
              <select
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="mi-customer" className="block text-[11px] uppercase tracking-[0.1em] mb-1" style={{ color: 'var(--text-3)' }}>Customer / group</label>
            ...
            <div>
              <label htmlFor="mi-event" className="block text-[11px] uppercase tracking-[0.1em] mb-1" style={{ color: 'var(--text-3)' }}>Event name</label>
          ...
          <div>
            <label htmlFor="mi-notes" className="block text-[11px] uppercase tracking-[0.1em] mb-1" style={{ color: 'var(--text-3)' }}>Notes</label>
          ...
          {formError && (
            <p className="text-[12px] rounded-[8px] px-3 py-2" style={{ background: 'rgba(248,113,113,0.1)', color: 'var(--st-f87171)' }} data-testid="fc-manual-error" role="alert">
              {formError}
            </p>
          )}

          <div className="flex items-center justify-end gap-3 pt-1">
            <button type="button" onClick={() => !busy && onClose()} className="text-[12px] font-semibold tracking-[0.08em] uppercase px-4 py-2 rounded-[10px] border transition-colors hover:bg-white/10" style={{ borderColor: 'var(--fg-a15)', color: 'var(--text-3)' }}>Cancel</button>
            <button type="submit" disabled={busy} className="text-[12px] font-semibold tracking-[0.08em] uppercase px-5 py-2 rounded-[10px] transition-colors disabled:opacity-50" style={{ background: 'var(--st-4ade80)', color: '#0a0a0a' }} data-testid="fc-manual-submit">
