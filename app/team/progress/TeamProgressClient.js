    ...
    <main className="max-w-[900px] mx-auto px-6 py-12">
      ...
      {initialTasks.length === 0 ? (
        <div className="rounded-[14px] p-12 text-center" style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.05)' }}>
          <p className="text-[15px]" style={{ color: '#8a8a8a' }}>Nothing assigned to you right now.</p>
        </div>
      ) : (
...
function Section({ title, tasks, todayStr, onOpen, assignees }) {
  ...
  return (
    <div>
      <div className="text-[11px] font-semibold tracking-[0.12em] mb-3" style={{ color: '#8a8a8a' }}>{title}</div>
        ...
        {tasks.map((t) => {
          ...
          return (
            <button key={t.id} onClick={() => onOpen(t)}
              ...
              <div className="text-[16px] font-bold mb-2" style={{ color: '#f0f0f0' }}>{t.title}</div>
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <StatusBadge value={t.status} />
                <span className="text-[12px]" style={{ color: c.overdue ? '#fca5a5' : '#8a8a8a' }}>Due {formatDate(t.due_date)}</span>
              </div>
