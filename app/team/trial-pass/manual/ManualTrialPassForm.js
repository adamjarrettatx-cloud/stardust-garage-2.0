...
export default function ManualTrialPassForm({ createdByEmail }) {
  ...
  if (result) {
    return (
      <div>
        ...
        >
          <div className="text-[10px] font-semibold tracking-[0.18em] mb-2" style={{ color: '#4ade80' }}>
            {result.existing ? 'PASS REISSUED' : 'PASS CREATED'}
          </div>
          <div className="text-[16px] font-semibold mb-1" style={{ color: '#ffffff' }}>
            {result.fullName}
          </div>
          <div className="text-[13px]" style={{ color: 'rgba(255,255,255,0.65)' }}>
            {result.email} · {result.phone}
          </div>
          <div className="text-[13px] mt-2" style={{ color: 'rgba(255,255,255,0.65)' }}>
            Good through <span style={{ color: '#ffffff', fontWeight: 600 }}>{result.expiresLabel}</span>
        ...
        >
          <div className="text-[10px] font-semibold tracking-[0.16em] mb-2" style={{ color: 'rgba(255,255,255,0.5)' }}>
            PASS URL
          </div>
          <a
            href={result.passUrl}
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={reset}
            className="flex-1 px-6 py-3.5 rounded-full text-[12px] font-semibold tracking-[0.14em]"
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
      ...
      {error && (
        <p className="text-[12px] mt-1 text-center" style={{ color: '#ff8a8a' }} role="alert">
          {error}
        </p>
      )}

      <p className="text-[11px] mt-4 text-center leading-[1.6]" style={{ color: 'rgba(255,255,255,0.4)' }}>
    ...
    <label className="block">
      ...
      <input
        ...
        style={{
          ...
          borderColor: bad ? '#c53030' : 'rgba(255,255,255,0.14)',
          color: '#ffffff',
        }}
      />
    </label>
