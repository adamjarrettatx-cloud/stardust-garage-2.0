...
export default function ActivateClient() {
  ...
  // /portal/auth/callback. Either way, by the time this resolves there is a
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // expired link. See /api/portal/resolve-identity.
      const res = await fetch('/api/portal/resolve-identity', { method: 'POST' });
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch('/api/portal/complete-activation', {
      method: 'POST',
    ...
    return shell(
      <div className="text-center">
        ...
        <p className="text-[14px] leading-[1.6] mb-8" style={{ color: '#a0a0a0' }}>
          We couldn&apos;t find a Stardust Garage invite for the account you signed in
  ...
  const roleUpper = roleLabel(profile.contact_type).toUpperCase();
